...
public final class AvatarNode: ASDisplayNode {
    ...
    public final class ContentNode: ASDisplayNode {
        ...
        func setPeerV2(
            ...
            case .roundedRect:
                self.imageNode.clipsToBounds = true
				// TODO: Ton - Avatar: edit avatar radius
                //self.imageNode.cornerRadius = displayDimensions.height * 0.25
				self.imageNode.cornerRadius = displayDimensions.height * 0.36
            case .bubble:
                break
            }
			...
			// TODO: Ton - ChatList: edit image Telegram on list
			if peerId == 777000  {
				self.imageNode.contents = UIImage(named: "Ton/AppIcon2")?.cgImage
				return
			}
			// TODO: Ton - ChatList: edit image Telegram Tip on list
			if peerId == 9814559261 {
				self.imageNode.contents = UIImage(named: "Ton/AppIconTip")?.cgImage
				return
			}
                ...
                if case .round = parameters.clipStyle {
					...
					context.addPath(
						UIBezierPath(
							...
							cornerRadius: floor(bounds.size.width * 0.36)).cgPath
					)
					context.clip()
