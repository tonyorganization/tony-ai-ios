...
private func settingsItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction, isExpanded: Bool) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    if let settings = data.globalSettings {
        if settings.premiumGracePeriod {
            items[.phone]!.append(PeerInfoScreenInfoItem(id: 0, title: "Your access to Tongram Premium will expire soon!", text: .markdown("Unfortunately, your latest payment didn't come through. To keep your access to exclusive features, please renew the subscription."), isWarning: true, linkAction: nil))
        ...
        if !settings.proxySettings.servers.isEmpty {
            ...
            if settings.proxySettings.enabled, let activeServer = settings.proxySettings.activeServer {
                ...
                case .mtp:
                    proxyType = presentationData.strings.SocksProxySetup_ProxyAppName
                case .socks5:
...
final class PeerInfoScreenNode: ViewControllerTracingNode, PeerInfoScreenNodeProtocol, ASScrollViewDelegate {
    ...
    init(controller: PeerInfoScreenImpl, context: AccountContext, peerId: PeerId, avatarInitiallyExpanded: Bool, isOpenedFromChat: Bool, nearbyPeerDistance: Int32?, reactionSourceMessageId: MessageId?, callMessages: [Message], isSettings: Bool, isMyProfile: Bool, hintGroupInCommon: PeerId?, requestsContext: PeerInvitationImportersContext?, profileGiftsContext: ProfileGiftsContext?, starsContext: StarsContext?, tonContext: StarsContext?, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>, switchToGiftsTarget: PeerInfoSwitchToGiftsTarget?, switchToStoryFolder: Int64?, initialPaneKey: PeerInfoPaneKey?, sharedMediaFromForumTopic: (EnginePeer.Id, Int64)?) {
        ...
        if self.isSettings {
            ...
            screenData = peerInfoScreenData(
				...
				hintGroupInCommon: hintGroupInCommon,
				existingRequestsContext: requestsContext,
				existingProfileGiftsContext: profileGiftsContext,
				existingProfileGiftsCollectionsContext: nil,
				chatLocation: self.chatLocation,
				chatLocationContextHolder: self.chatLocationContextHolder, sharedMediaFromForumTopic: self.sharedMediaFromForumTopic, privacySettings: self.privacySettings.get(), forceHasGifts: initialPaneKey == .gifts, switchToUpgradableGifts: switchToUpgradableGifts)
	...
	public func openBotApp(_ bot: AttachMenuBot) {
        guard let controller = self.controller else {
            return
        }
        ...
        let proceed: (Bool) -> Void = { [weak self] installed in
            ...
            let params = WebAppParameters(
				...
				peerId: peerId,
				botId: bot.peer.id,
				botName: bot.peer.compactDisplayTitle, botVerified: bot.peer.isVerified,
    ...
    private func openPhone(value: String, node: ASDisplayNode, gesture: ContextGesture?, progress: Promise<Bool>?) {
        ...
        let _ = (combineLatest(
            ...
            if case let .user(peer) = peer, let peerPhoneNumber = peer.phone, formattedPhoneNumber == formatPhoneNumber(context: strongSelf.context, number: peerPhoneNumber) {
                if !strongSelf.isMyProfile {
                    items.append(.action(ContextMenuActionItem(text: presentationData.strings.UserInfo_AppNameCall, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Call"), color: theme.contextMenu.primaryColor) }, action: { c, _ in
                        c?.dismiss {
                            telegramCallAction(false)
                        }
                    })))
                    items.append(.action(ContextMenuActionItem(text: presentationData.strings.UserInfo_AppNameVideoCall, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/VideoCall"), color: theme.contextMenu.primaryColor) }, action: { c, _ in
                        c?.dismiss {
...
//            }
    ...
    func containerLayoutUpdated(layout: ContainerViewLayout, navigationHeight: CGFloat, transition: ContainedViewLayoutTransition, additive: Bool = false) {
		...
		// TODO: Ton - Background: adapt
		self.backgroundImage.fillParent(color: self.presentationData.theme.list.plainBackgroundColor)
        ...
        if !self.isMediaOnly {
            ...
            var isFirst = true
            for (sectionId, sectionItems) in itemsMap {
    ...
    fileprivate func updateNavigation(transition: ContainedViewLayoutTransition, additive: Bool, animateHeader: Bool) {
        ...
        if self.isSettings, !(self.controller?.movingInHierarchy == true) {
			...
			let backgroundAlpha : CGFloat = 1.0
    ...
    func setup(topNavigationBar: NavigationBar, bottomNavigationBar: NavigationBar) {
        ...
        if let topNavigationBar = self.topNavigationBar, let bottomNavigationBar = self.bottomNavigationBar {
            ...
            if let headerView = bottomNavigationBar.customHeaderContentView as? ChatListHeaderComponent.View {
                if let previousBackButtonArrow = headerView.makeTransitionBackArrowView(accentColor: self.presentationData.theme.rootController.navigationBar.buttonColor) {
                    self.previousBackButtonArrow = previousBackButtonArrow
                    self.view.addSubview(previousBackButtonArrow)
                }
                if let previousBackButton = headerView.makeTransitionBackButtonView(accentColor: self.presentationData.theme.rootController.navigationBar.buttonColor) {
                    self.previousBackButton = previousBackButton
                    self.view.addSubview(previousBackButton)
                }
            } else {
                if let previousBackButtonArrow = bottomNavigationBar.makeTransitionBackArrowView(accentColor: self.presentationData.theme.rootController.navigationBar.buttonColor) {
                    self.previousBackButtonArrow = previousBackButtonArrow
                    self.view.addSubview(previousBackButtonArrow)
                }
                if let previousBackButton = bottomNavigationBar.makeTransitionBackButtonView(accentColor: self.presentationData.theme.rootController.navigationBar.buttonColor) {
                    self.previousBackButton = previousBackButton
                    self.view.addSubview(previousBackButton)
                }
            }
