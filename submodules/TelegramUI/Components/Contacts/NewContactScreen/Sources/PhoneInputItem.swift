	...
	func processNumberChange(_ number: String) -> Bool {
		...
		if let (country, _) = AuthorizationSequenceCountrySelectionController.lookupCountryIdByNumber(number, preferredCountries: self.preferredCountryIdForCode) {
			...
			if let mask = AuthorizationSequenceCountrySelectionController.lookupPatternByNumber(number, preferredCountries: self.preferredCountryIdForCode) {
				self.phoneInputNode.phoneNumberNode.textField.attributedPlaceholder = nil
				...
				self.phoneInputNode.phoneNumberNode.textField.attributedPlaceholder = NSAttributedString(string: item.strings.Login_PhonePlaceholder, font: Font.regular(17.0), textColor: item.theme.list.itemPlaceholderTextColor)
			}
		...
		return { item, params, neighbors in
			...
			return (layout, { [weak self] in
				if let strongSelf = self {
					...
					let phoneInputNode = strongSelf.phoneInputNode
					let dialingField = phoneInputNode.dialingTextNode.textField
					dialingField.textColor = item.theme.list.itemPrimaryTextColor
					dialingField.keyboardAppearance = item.theme.rootController.keyboardColor.keyboardAppearance
					dialingField.tintColor = item.theme.list.itemAccentColor
					
					
					let numberField =  phoneInputNode.phoneNumberNode.textField
					numberField.textColor = item.theme.list.itemPrimaryTextColor
					numberField.keyboardAppearance = item.theme.rootController.keyboardColor.keyboardAppearance
					numberField.tintColor = item.theme.list.itemAccentColor
					...
					phoneInputNode.frame = phoneInputFrame
					phoneInputNode.dialingTextNode.frame = countryCodeFrame.offsetBy(dx: -phoneInputFrame.minX, dy: -phoneInputFrame.minY)
					phoneInputNode.phoneNumberNode.frame = numberFrame.offsetBy(dx: -phoneInputFrame.minX, dy: -phoneInputFrame.minY)
					phoneInputNode.placeholderNode.frame = placeholderFrame.offsetBy(dx: -phoneInputFrame.minX, dy: -phoneInputFrame.minY + 8.0 + UIScreenPixel)
