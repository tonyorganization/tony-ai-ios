...
// TODO: Ton - Chat: Input panel node
public class ChatTextInputPanelNode: ChatInputPanelNode, ASEditableTextNodeDelegate, ChatInputTextNodeDelegate {
	...
	// TODO: Ton - AI: create button
	public var buttonAiNode: HighlightableButtonNode
	// TODO: Ton - AI: button press
	public var displayAIMenu: () -> Void = { }
    ...
    public func updateInputTextState(
		_ state: ChatTextInputState,
		keepSendButtonEnabled: Bool,
		extendedSearchLayout: Bool,
		accessoryItems: [ChatTextInputAccessoryItem],
		animated: Bool
	) {
        ...
        if let currentState = self.presentationInterfaceState {
            ...
            if updateAccessoryButtons {
                ...
                for item in accessoryItems {
					...
					// TODO: Ton - ChatInput: add accessory item
    ...
    public init(context: AccountContext, presentationInterfaceState: ChatPresentationInterfaceState, presentationContext: ChatPresentationContext?, presentController: @escaping (ViewController) -> Void) {
		...
		// TODO: Ton - AI: create button
		self.buttonAiNode = HighlightableButtonNode()
		...
		// TODO: Ton - ChatInput: rect (no round) container
		self.textInputBackgroundNode.backgroundColor = .clear
		self.textInputBackgroundNode.cornerRadius = 24
        ...
        self.startButton = SolidRoundedButtonNode(
			title: presentationInterfaceState.strings.Bot_Start,
			theme: SolidRoundedButtonTheme(theme: presentationInterfaceState.theme),
			glass: true,
			glassInset: true,
			height: 50.0,
			cornerRadius: 50.0 * 0.5,
			isShimmering: true
		)
...
//                    interfaceInteraction.finishMediaRecording(.dismiss)
		...
		// TODO: Ton - AI: create button
		self.buttonAiNode.addTarget(self, action: #selector(self.aiButtonPressed), forControlEvents: .touchUpInside)
		self.buttonAiNode.setImage(
			UIImage(named: "Ton/IconAi")?.withRenderingMode(.alwaysOriginal),
			for: .normal
		)
    }
    ...
    override public func updateLayout(
        ...
        self.startButton.frame = CGRect(
			origin: CGPoint(
				x: leftInset + floor((width - leftInset - rightInset - buttonSize.width) / 2.0),
				y: 6.0
			),
			size: buttonSize
		)
...
//                    self.menuButtonIconNode.enqueueState(.close, animated: true)
		...
		var currentButtonFrame: CGRect = CGRect.zero
		//var accessoryButtonWidth = 0.0
        for (item, button) in self.accessoryItemButtons.reversed() {
			//accessoryButtonWidth =  button.buttonWidth
			...
			let buttonFrame = CGRect(
				origin: CGPoint(
					x: nextButtonTopRight.x - buttonSize.width,
					y: nextButtonTopRight.y + floor((minimalInputHeight - buttonSize.height) / 2.0)
				),
				size: buttonSize
			)
            if button.superview == nil {
				...
				currentButtonFrame = button.frame
            } else {
                transition.updateFrame(layer: button.layer, frame: buttonFrame)
		...
		// TODO: Ton - AI: only add and layout button when has text
//		if let textInputNode = self.textInputNode, let attributedText = textInputNode.attributedText, attributedText.length != 0 {
//			let aiButtonFrame = currentButtonFrame.offsetBy(dx: -currentButtonFrame.size.width, dy: 0.0)
//			if self.buttonAiNode.supernode == nil {
//				self.textInputContainerBackgroundView.contentView.addSubview(self.buttonAiNode.view)
//				transition.updateFrame(layer: self.buttonAiNode.layer, frame: aiButtonFrame)
//				
//			} else {
//				transition.updateFrame(layer: self.buttonAiNode.layer, frame: aiButtonFrame)
//			}
//		}
		// TODO: Ton - AI: always add and layout button
		let aiButtonFrame = currentButtonFrame.offsetBy(dx: -currentButtonFrame.size.width, dy: 0.0)
		if self.buttonAiNode.supernode == nil {
			self.textInputContainerBackgroundView.contentView.addSubview(self.buttonAiNode.view)
			transition.updateFrame(layer: self.buttonAiNode.layer, frame: aiButtonFrame)
        ...
        self.attachmentButtonBackground.update(
			...
			isDark: interfaceState.theme.custom.isDark,
			tintColor: .init(
				kind: .panel,
				color: interfaceState.theme.chat.inputPanel.inputBackgroundColor.withMultipliedAlpha(0.7)
			),
			isInteractive: true,
			transition: ComponentTransition(transition)
		)
        ...
        if self.extendedSearchLayout {
            ...
            if (hasText || keepSendButtonEnabled && !mediaInputIsActive && !hasSlowModeButton) {
                if self.sendActionButtons.sendContainerNode.alpha.isZero && self.rightSlowModeInset.isZero {
					// TODO: Ton - AI: show and hide button
					//self.buttonAiNode.alpha = 1.0
					//alphaTransition.updateAlpha(node: self.buttonAiNode, alpha: 1.0, completion: nil)
                ...
                if !self.sendActionButtons.sendContainerNode.alpha.isZero {
					...
					// TODO: Ton - AI: hide button
					//alphaTransition.updateAlpha(node: self.buttonAiNode, alpha: 0.0, completion: nil)
                    blurTransitionOut.animateBlur(layer: self.sendActionButtons.sendContainerNode.layer, fromRadius: 0.0, toRadius: sendButtonBlurOut)
                    if let sendButtonRadialStatusNode = self.sendActionButtons.sendButtonRadialStatusNode {
                        alphaTransition.updateAlpha(node: sendButtonRadialStatusNode, alpha: 0.0)
                    }
	...
	// TODO: Ton - ChatInput: accessoryItemButtonPressed
	...
	// TODO: Ton - AI: button press
	@objc private func aiButtonPressed(_ button: UIView) {
		self.displayAIMenu()
	}
