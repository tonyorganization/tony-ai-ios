...
//				wallpaper: interfaceState.chatWallpaper
//			)
            
			let textColor =  self.currentTheme?.custom.primaryTextColor ?? .black
            ...
            self.subtitleNode.attributedText = NSAttributedString(
				string: interfaceState.strings.Conversation_EncryptedDescriptionTitle,
				font: messageFont,
				textColor: textColor
			)
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, subject: ChatEmptyNode.Subject, size: CGSize, leftInset: CGFloat, rightInset: CGFloat, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        if self.currentTheme !== interfaceState.theme || self.currentStrings !== interfaceState.strings {
            ...
            self.iconNode.image = generateTintedImage(
				...
				color: interfaceState.theme.custom.primaryTextColor
			)
