...
public class ChatMessageTextBubbleContentNode: ChatMessageBubbleContentNode {
	...
	// TODO: Ton - Chat: Message bubble layout
		...
		return { item, layoutConstants, _, _, _, _ in
			...
			return (contentProperties, nil, CGFloat.greatestFiniteMagnitude, { constrainedSize, position in
				...
				if let entities {
					...
					// TODO: Ton - Chat: Modify Telegram text in Telegram login item
					if item.message.author?.id.toInt64() == 777000 {
						rawText = rawText.replacingOccurrences(of: "Telegram", with: "Tongram")
					}
					
					// TODO: Ton - Chat: Message bubble text
					attributedText = stringWithAppliedEntities(
						rawText,
						entities: entities,
						baseColor: messageTheme.primaryTextColor,
						linkColor: messageTheme.linkTextColor,
						baseQuoteTintColor: mainColor,
						baseQuoteSecondaryTintColor: secondaryColor,
						baseQuoteTertiaryTintColor: tertiaryColor,
						codeBlockTitleColor: codeBlockTitleColor,
						codeBlockAccentColor: codeBlockAccentColor,
						codeBlockBackgroundColor: codeBlockBackgroundColor,
						baseFont: textFont,
						linkFont: textFont,
						boldFont: item.presentationData.messageBoldFont,
						italicFont: item.presentationData.messageItalicFont,
						boldItalicFont: item.presentationData.messageBoldItalicFont,
						fixedFont: item.presentationData.messageFixedFont,
						blockQuoteFont: item.presentationData.messageBlockQuoteFont,
		...
		if value {
			if self.textSelectionNode == nil, let item = self.item, let rootNode = item.controllerInteraction.chatControllerNode() {
				...
				let textSelectionNode = TextSelectionNode(
					...
					knob: knobColor,
					isDark: item.presentationData.theme.theme.custom.isDark
				),
					strings: item.presentationData.strings,
					textNode: self.textNode.textNode,
