    ...
    outer: for (message, itemAttributes) in item.content {
        for attribute in message.attributes {
            if let attribute = attribute as? RestrictedContentMessageAttribute, attribute.platformText(platform: "ios", contentSettings: item.context.currentContentSettings.with { $0 }) != nil {
                ...
                addedPriceInfo = true
			}  else if let att = attribute as? TranslateTextMessageAttribute {
				// TODO: Ton - Translate
				print("\(att)")
			}
        }
            ...
            if !skipText {
                if case .group = item.content, !isFile {
                    ...
                    if isMediaInverted {
						...
						if isFile {
							result.append((message, ChatMessageTextBubbleContentNode.self, itemAttributes, BubbleItemAttributes(isAttachment: false, neighborType: .text, neighborSpacing:  .condensed)))
						}else{
							...
							// TODO: Ton - Translate: Add TranslationContentNode
							let incoming = message.effectivelyIncoming(item.context.account.peerId)
							
							if incoming && !item.presentationData.isPreview {
								result.append(
									(message,
									 TranslationContentNode.self,
									 itemAttributes,
									 BubbleItemAttributes(
										isAttachment: true,
										neighborType: .text,   // or .footer
										neighborSpacing: .default
									 ))
								)
							}
...
public class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    public class ContentContainer {
        ...
        fileprivate func update(size: CGSize, contentOrigin: CGPoint, selectionInsets: UIEdgeInsets, index: Int, presentationData: ChatPresentationData, graphics: PrincipalThemeEssentialGraphics, backgroundType: ChatMessageBackgroundType, presentationContext: ChatPresentationContext, mediaBox: MediaBox, messageSelection: Bool?) {
			...
			// TODO: Ton - Chat: modify bubble color
    ...
    private func contextContentNodeForLink(_ link: String, rects: [CGRect]?) -> ContextExtractedContentContainingNode? {
		...
		// TODO: Ton - Chat: edit bubble color
		backgroundNode.backgroundColor = (incoming ? item.presentationData.theme.theme.custom.incomingBubble : item.presentationData.theme.theme.custom.outgoingBubble).first ?? .black
