                ...
                return (suggestedBoundingWidth, { boundingWidth in
                    ...
                    return (boundingSize, { [weak self] animation, synchronousLoad, _ in
                        if let strongSelf = self {
                            ...
                            strongSelf.separatorNode.frame = CGRect(
								origin: CGPoint(x: layoutConstants.bubble.strokeInsets.left,y: -3.0 + topSeparatorOffset),
								size: CGSize(width: boundingWidth - layoutConstants.bubble.strokeInsets.left - layoutConstants.bubble.strokeInsets.right, height: UIScreenPixel)
							)
