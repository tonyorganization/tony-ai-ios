...
public final class TelegramRootController: NavigationController, TelegramRootControllerInterface {
	...
	public func addRootControllers(showCallsTab: Bool) {
		...
		// TODO: Ton - Discover page: create view controller
		let discoverController = DiscoverController(context: self.context)
		discoverController.discoverItemClick = { id in
			if id == 1 {
				self.openTonTVImpl()
				return
			}
		...
		// TODO: Ton - Discover page: set view controller
		...
		//tabBarController.openTonTV = {self.openTonTVImpl()}
		//tabBarController.openTonApp = {self.openTonAppImpl()}
	}
	...
	// TODO: Ton - Discover page: TV Click
	public func openTonTVImpl(){
		print("openTonTV: http://t.me/tontv_official_bot/app")
		let _ = (
			self.context.engine.peers.resolvePeerByName(
				name: "tontv_official_bot",
				referrer: nil
			)
			...
			|> deliverOnMainQueue).start(next: { peer in
				print("openAppByName: deliverOnMainQueue")
				guard let peer else {
					return
				}
				guard let controller = self.viewControllers.last as? ViewController else {
				...
				print("openAppByName: got peer \(peer)")
	...
	// TODO: Ton - Discover page: Mini Apps Click
	public func openTonAppImpl(){
		print("openTonApp: https://t.me/Tongram_center_bot/Tongram_Apps_Center")
		
		let _ = (
			self.context.engine.peers.resolvePeerByName(
				name: "Tongram_center_bot",
				referrer: nil
			)
			|> mapToSignal { result -> Signal<EnginePeer?, NoError> in
				guard case let .result(result) = result else {
			...
			|> deliverOnMainQueue).start(next: { peer in
				...
				self.context.sharedContext.openWebApp(
					...
					threadId: nil,
					buttonText: "",
					url: "https://t.me/Tongram_center_bot/Tongram_Apps_Center?mode=fullscreen",
					simple: false,
					source: .menu,
					skipTermsOfService: true,
					payload: nil,
	...
	// TODO: Ton - Discover page: Wallet Click
	public func openWallet(){
		self.openBotApp(botId: 1985737506)
	}
	

	
	// TODO: Ton - Bot open bot
	private func openBotApp(botId: Int64) {
		
		let getBotTask = self.context.engine.messages.attachMenuBots()
		|> mapToSignal { bots -> Signal<AttachMenuBot?, NoError> in
			if let walletBot = bots.first(where: { $0.peer.id.toInt64() == botId }) {
				return .single(walletBot)
			}
			return .complete()
		}
		
		let getAppSettingsTask = self.context.account.postbox.transaction { transaction -> BotAppSettings? in
			let peerId = self.context.account.peerId
			guard let cachedData = transaction.getPeerCachedData(peerId: peerId) else {
				return nil
			}
			
			var appSettings: BotAppSettings? = nil
			if let settings = cachedData as? CachedUserData {
		...
		let _ = combineLatest(
			...
			getBotTask,
			getAppSettingsTask
		).start(next: { [weak self] bot, settings in
			guard let strongSelf = self else { return }
			guard let strongBot = bot else { return }
			strongSelf.proceedBot(
				bot: strongBot,
	...
	private func proceedBot(
		...
		let params = WebAppParameters(
			...
			botId: bot.peer.id,
			botName: bot.peer.compactDisplayTitle,
			botVerified: bot.peer.isVerified,
			botAddress: bot.peer.addressName ?? "",
			appName: "",
			url: nil,
			queryId: nil,
			payload: nil,
			buttonText: nil,
			keepAliveSignal: nil,
			forceHasSettings: bot.flags.contains(.hasSettings),
			fullSize: true,
			appSettings: appSettings
		)
		let webAppController = standaloneWebAppController(
			context: self.context,
			updatedPresentationData: nil,
			params: params,
			threadId: nil,
			openUrl: { url, concealed, forceUpdate, commit in
				self.openUrl(
					url : url,
					concealed: concealed,
		...
		webAppController.navigationPresentation = .flatModal
		let controller = self.viewControllers.last as? ViewController
		controller?.push(webAppController)

		
	}
	
	// TODO: Ton - alertComingSoon
	public func alertComingSoon(){
		guard let controller = self.viewControllers.last as? ViewController else {
			return
		}
	...
	//---------------
	private func _openBotApp(botId: Int64) {
		...
		let _ = combineLatest(
			...
			if !strongBot.flags.contains(.notActivated) && !strongBot.flags.contains(.showInSettingsDisclaimer) {
				strongSelf._proceedBot(
					bot: strongBot,
					appSettings: settings,
					installed: false
				)
			...
			let alertController = webAppTermsAlertController(
				context: strongSelf.context,
				updatedPresentationData: nil,
				bot: strongBot,
				completion: { [weak self] allowWrite in
					guard let strongSelf = self else {
						return
					}
					let showInstalledTooltip = !strongBot.flags.contains(.showInSettingsDisclaimer)
					if strongBot.flags.contains(.showInSettingsDisclaimer) {
						let _ = strongSelf.context.engine.messages.acceptAttachMenuBotDisclaimer(botId: strongBot.peer.id).startStandalone()
					}
					if strongBot.flags.contains(.notActivated) {
						let _ = (strongSelf.context.engine.messages.addBotToAttachMenu(botId: strongBot.peer.id, allowWrite: allowWrite)
								 |> deliverOnMainQueue).startStandalone(error: { _ in
						}, completed: {
							strongSelf._proceedBot(
								bot: strongBot,
								appSettings: settings,
								installed: showInstalledTooltip
							)
						})
					} else {
						strongSelf._proceedBot(
			...
			let controller = strongSelf.viewControllers.last as? ViewController
			controller?.present(alertController, in: .window(.root))
		})
		
	}
	
	private func _proceedBot(
		bot: AttachMenuBot,
		appSettings: BotAppSettings?,
		installed : Bool
	){
		...
		if installed {
			Queue.mainQueue().after(0.3, {
				...
				if bot.flags.contains(.showInSettings) {
					text = self.presentationData.strings
						.WebApp_ShortcutsSettingsAdded(bot.peer.compactDisplayTitle)
						.string
				} else {
					text = self.presentationData.strings  .WebApp_ShortcutsAdded(bot.peer.compactDisplayTitle)
						.string
				}
				let controller = self.viewControllers.last as? ViewController
				controller?.present(
					UndoOverlayController(
						...
						content: .succeed(text: text, timeout: 5.0, customUndoText: nil),
	...
	private func openUrl(
		...
		//					context: context,
		//					peerId: peerId, url: url,
		//					concealed: concealed, present: { [weak self] c in
		//						//self?.controller?.present(c, in: .window(.root))
		//					},
		//					openResolved: { result in
		//						var navigationController: NavigationController?
		//						if let current = self as? NavigationController {
