        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            ...
            if let peer {
                if peer.id == self.context.account.peerId {
                    ...
                    items.append(
                        .action(ContextMenuActionItem(text: self.presentationData.strings.Chat_Context_Phone_AppNameVoiceCall, icon: { theme in return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Call"), color: theme.contextMenu.primaryColor) }, action: { [weak self]  _, f in
                            f(.default)
                    ...
                    items.append(
                        .action(ContextMenuActionItem(text: self.presentationData.strings.Chat_Context_Phone_AppNameVideoCall, icon: { theme in return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/VideoCall"), color: theme.contextMenu.primaryColor) }, action: { [weak self]  _, f in
                            f(.default)
                ...
                items.append(
                    .action(ContextMenuActionItem(text: self.presentationData.strings.Chat_Context_Phone_InviteToAppName, icon: { theme in return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Telegram"), color: theme.contextMenu.primaryColor) }, action: { [weak self]  _, f in
                        f(.default)
            ...
            if let peer {
                ...
                items.append(
                    .action(ContextMenuActionItem(text: self.presentationData.strings.Chat_Context_Phone_NotOnAppName, textLayout: .multiline, textFont: .small, icon: { _ in return nil }, action: emptyAction))
                )
            }
