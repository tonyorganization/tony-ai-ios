...
//                }
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            ...
            if let peer {
                ...
                items.append(
                    .action(ContextMenuActionItem(text: self.presentationData.strings.Chat_Context_Username_NotOnAppName, textLayout: .multiline, textFont: .small, icon: { _ in return nil }, action: emptyAction))
                )
            }
