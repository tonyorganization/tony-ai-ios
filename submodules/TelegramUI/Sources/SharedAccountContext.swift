...
public final class SharedAccountContextImpl: SharedAccountContext {
    ...
    init(mainWindow: Window1?, sharedContainerPath: String, basePath: String, encryptionParameters: ValueBoxEncryptionParameters, accountManager: AccountManager<TelegramAccountManagerTypes>, appLockContext: AppLockContext, notificationController: NotificationContainerController?, applicationBindings: TelegramApplicationBindings, initialPresentationDataAndSettings: InitialPresentationDataAndSettings, networkArguments: NetworkInitializationArguments, hasInAppPurchases: Bool, rootPath: String, legacyBasePath: String?, apsNotificationToken: Signal<Data?, NoError>, voipNotificationToken: Signal<Data?, NoError>, firebaseSecretStream: Signal<[String: String], NoError>, setNotificationCall: @escaping (PresentationCall?) -> Void, navigateToChat: @escaping (AccountRecordId, PeerId, MessageId?, Bool) -> Void, displayUpgradeProgress: @escaping (Float?) -> Void = { _ in }, appDelegate: AppDelegate?) {
		...
		// TODO: Ton - Theme: Glass disable
        //GlassBackgroundView.isUseGlass = immediateExperimentalUISettingsValue.with({ $0.fakeGlass })
...
//        return controller
		return simpleTextAlert(text: "Coming soon")
    }
...
//        return controller
	...
	// TODO: Ton - Alert: textAlert
	public func simpleTextAlert(text: String) -> AlertController{
	...
	public func confirmTextAlert(text: String,action: @escaping () -> Void) -> AlertController {
		return textAlertController(
			sharedContext: self,
			title: nil,
			text: text,
	...
	// TODO: Ton - Translate: tonfetch
	public func postTranslateText(text: String, language: String) -> Signal<String?, NoError> {
		return apiRequest(
			method: "POST",
			endpoint: "ton-api/api/v1/translate-proxy",
			body: [
				"text": text,
			...
			onResponse: { res in
				if let result = res["result"] as? [String: Any],
	...
	public func fetchLanguages() -> Signal<[LanguageItemInfo]?, NoError> {
		return apiRequest(
			method: "GET",
			endpoint: "ton-api/api/v1/list-languages",
			body: nil,
			onResponse: { res in
				guard let items = res["array"] as?  [[String: Any]] else {
					return [LanguageItemInfo]()
				}
				let languageList: [LanguageItemInfo] = items.compactMap { dict in
					return LanguageItemInfo(
						code: dict["code"] as? String ?? "",
						name: dict["name"] as? String ?? "",
						nativeName: dict["nativeName"] as? String ?? "",
					)
				}
				return languageList
			}
	...
	public func apiRequest<T>(
		...
		endpoint: String,
		body: [String: Any]?,
		onResponse: @escaping ([String: Any]) -> T
	) -> Signal<T?, NoError>{
		...
		return Signal { subscriber in
			...
			if let sBody = body{
				if !sBody.isEmpty {
					request.httpBody = try? JSONSerialization.data(withJSONObject: sBody)
				}
			}
			let task = URLSession.shared.dataTask(with: request) { data, _, _ in
				...
				if let response = data {
					...
					if let obj = try? JSONSerialization.jsonObject(with: response) as? [String: Any] {
						let result = onResponse(obj)
						subscriber.putNext(result)
						subscriber.putCompletion()
						return
					}
					if let array = try? JSONSerialization.jsonObject(with: response) as? [[String: Any]] {
						let result = onResponse([
							"array" : array
						])
						subscriber.putNext(result)
						subscriber.putCompletion()
