...
public final class ChatControllerImpl: TelegramBaseController, ChatController, GalleryHiddenMediaTarget, UIDropInteractionDelegate {    
    ...
    func updateNavigationBarPresentation() {
        ...
        if let forcedNavigationBarTheme = self.forcedNavigationBarTheme {
            ...
            navigationBarTheme = NavigationBarTheme(
				rootControllerTheme: forcedNavigationBarTheme,
            ...
            navigationBarTheme = NavigationBarTheme(
				rootControllerTheme: defaultDarkPresentationTheme,
				//hideBackground: self.context.sharedContext.immediateExperimentalUISettings.playerEmbedding ? true : false,
				hideBadge: true
			)
        } else {
            presentationTheme = self.presentationData.theme
	...
	// TODO: Ton - AI: present ai menu
	
	private var tranlatedText: String = ""
	private var currentText: String = ""
	
	public func presentAIMenu()  {
#if targetEnvironment(simulator)
		//self.presentAiController2()
		self.presentAiControllerLagacy()
#else
		self.presentAiControllerLagacy()
#endif
	...
	private func presentAiControllerLagacy() {
		self.chatDisplayNode.dismissInput()
		let inputIsActive = self.presentationInterfaceState.inputMode == .text
		
		let chatState = self.chatDisplayNode.chatPresentationInterfaceState.interfaceState
		var inputState: ChatTextInputState
		
		if chatState.editMessage != nil && chatState.postSuggestionState != nil {
			inputState = chatState.effectiveInputState
		} else {
			inputState = chatState.composeInputState
		}
		let s = expandedInputStateAttributedString(inputState.inputText).string
		let ctl = AiController(
			context: self.context,
			userInputText: s,
			translatedText: s == self.currentText ? self.tranlatedText : nil
		)
		self.currentText = s
		ctl.onTextTranslated = { [weak self] text in
			self?.tranlatedText = text
		}
		ctl.onTextApplied = { [weak self] text in
			Queue.mainQueue().async {
				self?.updateChatPresentationInterfaceState(animated: true, interactive: false, { current in
					return current.updatedInterfaceState { interfaceState in
						// read current compose text and append
						
						let newText = NSMutableAttributedString(attributedString: NSAttributedString(string: text))
						
						return interfaceState.withUpdatedComposeInputState(ChatTextInputState(inputText: newText))
					}.updatedInputMode({ _ in .text })
				})
			}
		}
		let presentModal = {
			self.present(ctl, in: .window(.root))
		}
		if inputIsActive {
			Queue.mainQueue().after(0.15, {
				presentModal()
	...
	private func presentAiController2() {
		self.chatDisplayNode.dismissInput()
		let inputIsActive = self.presentationInterfaceState.inputMode == .text
