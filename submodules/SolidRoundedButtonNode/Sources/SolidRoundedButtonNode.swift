...
public final class SolidRoundedButtonNode: ASDisplayNode {
        ...
        didSet {
            if let animation = self.animation {
                if animation != oldValue {
					...
					// TODO: Ton - Theme: button shimmer animation color
    ...
    public init(
		...
		let sHeight = height ?? 50
		self.buttonHeight = sHeight
		let sRadius = cornerRadius ?? 25
        self.buttonCornerRadius = sRadius
        self.title = title
        ...
        if theme.backgroundColors.count > 1 {
            ...
            self.buttonBackgroundNode.image = generateGradientImage(size: CGSize(width: 200.0, height: sHeight), colors: theme.backgroundColors, locations: locations, direction: .horizontal)
            
            let buttonBackgroundAnimationView = UIImageView()
            buttonBackgroundAnimationView.image = generateGradientImage(size: CGSize(width: 200.0, height: sHeight), colors: theme.backgroundColors, locations: locations, direction: .horizontal)
            self.buttonBackgroundAnimationView = buttonBackgroundAnimationView
        }
        
        self.buttonBackgroundNode.cornerRadius = sRadius
