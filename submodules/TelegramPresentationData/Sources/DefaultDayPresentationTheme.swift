...
// TODO: Ton - Theme: color resources - light theme 
let lightCustom = CustomTheme(
	isDark: false,
	accentColor : UIColor(rgb: 0x7E99D5),
	accent2Color : UIColor(rgb: 0xAFC8FF),

	
	aiButtonColor: UIColor(rgb: 0xD9D9D9),
	aiButtonDisabledColor: UIColor(rgb: 0xF2F2F2),
	aiInputBackground : UIColor(rgb: 0xFFFFFF),
	aiInputTextColor : UIColor(rgb: 0x000000),
	aiInputHolderColor : UIColor(rgb: 0x666666),
	aiTranslatedColor : UIColor(rgb: 0x000000),
	
	backgroundColor : UIColor(rgb: 0xDFE8F4),
	blurredBackgroundColor : UIColor(rgb: 0xDFE8F4),
	
	buttonTextColor: UIColor(rgb: 0x000000),
	chatDateHeaderTextColor: UIColor(rgb: 0x000000),
	defaultBlock : UIColor(rgb: 0xFFFFFF),
	dimColor: UIColor(rgb: 0x000a26, alpha: 0.3),
	
	incomingBubble : [UIColor(rgb: 0x1A1F29),UIColor(rgb: 0x1A1F29)],
	incomingSecondaryText : UIColor(rgb: 0x5B5D61),
	incomingText : UIColor(rgb: 0x000000),
	inputBackground : UIColor(rgb: 0xCAD4EA),
	itemHighlightColor : UIColor(rgb: 0xCCD8EE),

	linkTextColor : UIColor(rgb: 0x3f88f7),
	modalBackground : UIColor(rgb: 0xFFFFFF),
	
	navBarColor : UIColor(rgb: 0xDFE8F4),
	navBarButtonIconColor : UIColor(rgb: 0x000000),
	navBarButtonTextColor : UIColor(rgb: 0x000000),
	navBarTitleColor : UIColor(rgb: 0x000000),
	
	outgoingBubble : [UIColor(rgb: 0xAFC8FF),UIColor(rgb: 0xAFC8FF)],
	outgoingSecondaryText : UIColor(rgb: 0x586480),
	outgoingText : UIColor(rgb: 0x000000),
	
	primaryTextColor : UIColor(rgb: 0x000000),
	secondaryTextColor : UIColor(rgb: 0x5B5D61),
	
	selectedButton: UIColor(rgb: 0x7E99D5),
	unselectButton: UIColor(rgb: 0xFFFFFF),
	unselectButton2: UIColor(rgb: 0x80828A),
	selectedButtonText: UIColor(rgb: 0x000000),
	unselectButtonText: UIColor(rgb: 0x828284)
)
...
public let defaultServiceBackgroundColor = UIColor(rgb: 0xFFFFFF, alpha: 0.4)

public let defaultPresentationTheme = makeDefaultDayPresentationTheme(
	serviceBackgroundColor: defaultServiceBackgroundColor,
	day: false,
	preview: false,
)

// TODO: Ton - Theme: Accent color
public let defaultDayAccentColor = UIColor(rgb: 0x7E99D5)
...
public func customizeDefaultDayTheme(theme: PresentationTheme, editing: Bool, title: String?, accentColor: UIColor?, outgoingAccentColor: UIColor?, backgroundColors: [UInt32], bubbleColors: [UInt32], animateBubbleColors: Bool?, wallpaper forcedWallpaper: TelegramWallpaper? = nil, serviceBackgroundColor: UIColor?) -> PresentationTheme {
	...
	if bubbleColors.isEmpty, editing {
		if day {
			let accentColor = accentColor ?? customTheme.accentColor
			...
			if let accentColor = accentColor, !accentColor.alpha.isZero {
				...
				bubbleColors = [ customTheme.accent2Color.rgb ]
				suggestedWallpaper = .gradient(TelegramWallpaper.Gradient(id: nil, colors: defaultBuiltinWallpaperGradientColors.map(\.rgb), settings: WallpaperSettings()))
				outgoingAccent = outgoingAccentColor
			}
		}
	...
	if let accentColor = accentColor {
		intro = intro.withUpdated(accentTextColor: UIColor(rgb: 0x7C99D6))
		rootController = rootController.withUpdated(
			...
			navigationBar: rootController.navigationBar.withUpdated(
				buttonColor: UIColor(rgb: 0x000000),
				accentTextColor: UIColor(rgb: 0x000000)
			),
	...
	if !bubbleColors.isEmpty {
		...
		if lightnessColor.lightness > 0.705 {
			...
			outgoingPrimaryTextColor = customTheme.outgoingText
			...
			outgoingPrimaryTextColor = customTheme.outgoingText
			outgoingSecondaryTextColor = customTheme.outgoingSecondaryText
			outgoingAccentTextColor = outgoingPrimaryTextColor
			outgoingLinkTextColor = outgoingPrimaryTextColor
			...
			outgoingCheckColor = outgoingPrimaryTextColor
		}
	}
	outgoingPrimaryTextColor = customTheme.outgoingText
	outgoingSecondaryTextColor = customTheme.outgoingSecondaryText
	outgoingLinkTextColor = customTheme.linkTextColor
	outgoingScamColor =  customTheme.outgoingText
	outgoingCheckColor = customTheme.outgoingSecondaryText
	...
	chat = chat.withUpdated(
			...
			incoming: chat.message.incoming.withUpdated(
				bubble: chat.message.incoming.bubble.withUpdated(
					withWallpaper: chat.message.incoming.bubble.withWallpaper.withUpdated(
						// TODO: Ton - Theme: Incoming Bubble color edit
						fill: customTheme.incomingBubble,
				...
				primaryTextColor: customTheme.incomingText,
				secondaryTextColor: customTheme.incomingSecondaryText,
			...
			outgoing: chat.message.outgoing.withUpdated(
				bubble: chat.message.outgoing.bubble.withUpdated(
					withWallpaper: chat.message.outgoing.bubble.withWallpaper.withUpdated(
						// TODO: Ton - Theme: Outgoing Bubble color edit
						fill: customTheme.outgoingBubble,
				...
				primaryTextColor: customTheme.outgoingText,
				secondaryTextColor: customTheme.secondaryTextColor,
		...
		serviceMessage: serviceBackgroundColor.flatMap {
			chat.serviceMessage.withUpdated(
				components: chat.serviceMessage.components.withUpdated(
					withCustomWallpaper: chat.serviceMessage.components.withCustomWallpaper.withUpdated(fill: $0, dateFillStatic: $0, dateFillFloating: $0.withAlphaComponent($0.alpha * 0.6667)
																									   )
				),
...
// TODO: Ton - Theme: default day theme
public func makeDefaultDayPresentationTheme(extendingThemeReference: PresentationThemeReference? = nil, serviceBackgroundColor: UIColor?, day: Bool, preview: Bool) -> PresentationTheme {
	...
	let intro = PresentationThemeIntro(
		...
		primaryTextColor: UIColor(rgb: 0x000000),
		accentTextColor: UIColor(rgb: 0x7C99D6),
	...
	let rootNavigationBar = PresentationThemeRootNavigationBar(
		...
		accentTextColor: UIColor(rgb: 0x000000),
		blurredBackgroundColor: customTheme.blurredBackgroundColor,
		opaqueBackgroundColor: customTheme.navBarColor.withAlphaComponent(0.14),
	...
	let rootTabBar = PresentationThemeRootTabBar(
		backgroundColor: customTheme.navBarColor,
		separatorColor: UIColor(rgb: 0xb2b2b2),
		iconColor: UIColor(rgb: 0x959595),
		selectedIconColor: customTheme.accentColor,
		textColor: UIColor(rgb: 0x000000),
		selectedTextColor: customTheme.accentColor,
		badgeBackgroundColor: UIColor(rgb: 0xff3b30),
		badgeStrokeColor: UIColor(rgb: 0xff3b30),
		badgeTextColor: UIColor(rgb: 0xFFFFFF)
	)
	...
	let list = PresentationThemeList(
		blocksBackgroundColor: customTheme.backgroundColor,
		modalBlocksBackgroundColor: customTheme.defaultBlock,
		plainBackgroundColor: customTheme.backgroundColor,
		modalPlainBackgroundColor: customTheme.backgroundColor,
		...
		itemAccentColor: customTheme.accentColor,
		itemHighlightedColor: customTheme.itemHighlightColor,
		itemDestructiveColor: UIColor(rgb: 0xff3b30),
		itemPlaceholderTextColor: UIColor(rgb: 0xc8c8ce),
		// Discover session block background
		itemBlocksBackgroundColor: UIColor(rgb: 0xFFFFFF),
		itemModalBlocksBackgroundColor: customTheme.defaultBlock,
		// List item press
		itemHighlightedBackgroundColor: customTheme.itemHighlightColor,
		...
		itemDisclosureActions: PresentationThemeItemDisclosureActions(
			...
			accent: PresentationThemeFillForeground(fillColor: customTheme.accentColor, foregroundColor: UIColor(rgb: 0xFFFFFF)),
		...
		itemBarChart: PresentationThemeItemBarChart(color1: customTheme.accentColor, color2: UIColor(rgb: 0xc8c7cc), color3: UIColor(rgb: 0xf2f1f7)),
	...
	let chatList = PresentationThemeChatList(
		...
		itemSelectedBackgroundColor: customTheme.itemHighlightColor,
		titleColor: UIColor(rgb: 0x000000),
		secretTitleColor: UIColor(rgb: 0x00b12c),
		dateTextColor: UIColor(rgb: 0x000000),
		...
		checkmarkColor: day ? customTheme.accentColor : UIColor(rgb: 0x21c004),
		...
		unreadBadgeActiveBackgroundColor: customTheme.accentColor,
		...
		verifiedIconFillColor: customTheme.accentColor,
	...
	let message = PresentationThemeChatMessage(
		incoming: PresentationThemePartedColors(
			bubble: PresentationThemeBubbleColor(
				withWallpaper: PresentationThemeBubbleColorComponents(
					...
					shadow: nil,
					reactionInactiveBackground: customTheme.accentColor.withMultipliedAlpha(0.1),
					reactionInactiveForeground: customTheme.accentColor,
					reactionActiveBackground: customTheme.accentColor,
					reactionActiveForeground: .clear,
					reactionStarsInactiveBackground: UIColor(rgb: 0xFEF1D4),
					reactionStarsInactiveForeground: UIColor(rgb: 0xD3720A),
					reactionStarsActiveBackground: UIColor(rgb: 0xFFBC2E),
			...
			linkHighlightColor: customTheme.accentColor.withAlphaComponent(0.3),
			scamColor: UIColor(rgb: 0xff3b30),
			textHighlightColor: UIColor(rgb: 0xffe438),
			accentTextColor: customTheme.accentColor,
			accentControlColor: customTheme.accentColor,
			accentControlDisabledColor: UIColor(rgb: 0x525252, alpha: 0.6),
			mediaActiveControlColor: customTheme.accentColor,
			...
			polls: PresentationThemeChatBubblePolls(radioButton: UIColor(rgb: 0xc8c7cc), radioProgress: customTheme.accentColor, highlight: customTheme.accentColor.withAlphaComponent(0.08), separator: UIColor(rgb: 0xc8c7cc), bar: customTheme.accentColor, barIconForeground: .white, barPositive: UIColor(rgb: 0x2dba45), barNegative: UIColor(rgb: 0xFE3824)),
			...
			actionButtonsTextColor: PresentationThemeVariableColor(color: UIColor(rgb: 0xFFFFFF)), textSelectionColor: customTheme.accentColor.withAlphaComponent(0.2), textSelectionKnobColor: customTheme.accentColor),
		outgoing: PresentationThemePartedColors(
			...
			primaryTextColor: customTheme.outgoingText,
			secondaryTextColor: customTheme.outgoingSecondaryText,
			linkTextColor: UIColor(rgb: 0x004bad),
			linkHighlightColor: UIColor(rgb: 0x004bad),
			scamColor: UIColor(rgb: 0xff3b30),
			textHighlightColor: UIColor(rgb: 0xffe438),
			...
			mediaControlInnerBackgroundColor: customTheme.accentColor,
		...
		selectionControlColors: PresentationThemeFillStrokeForeground(fillColor: customTheme.accentColor, strokeColor: UIColor(rgb: 0xc7c7cc), foregroundColor: UIColor(rgb: 0xFFFFFF)),
	...
	let messageDay = PresentationThemeChatMessage(
		incoming: PresentationThemePartedColors(
			...
			mediaControlInnerBackgroundColor: UIColor(rgb: 0xFFFFFF),
			pendingActivityColor: UIColor(rgb: 0x525252, alpha: 0.6),
			fileTitleColor: customTheme.accentColor,
			fileDescriptionColor: UIColor(rgb: 0x999999),
			fileDurationColor: UIColor(rgb: 0x525252, alpha: 0.6),
			mediaPlaceholderColor: UIColor(rgb: 0xFFFFFF).withMultipliedBrightnessBy(0.95),
			polls: PresentationThemeChatBubblePolls(radioButton: UIColor(rgb: 0xc8c7cc), radioProgress: customTheme.accentColor, highlight: customTheme.accentColor.withAlphaComponent(0.12), separator: UIColor(rgb: 0xc8c7cc), bar: customTheme.accentColor, barIconForeground: .white, barPositive: UIColor(rgb: 0x00A700), barNegative: UIColor(rgb: 0xFE3824)),
			...
			actionButtonsStrokeColor: PresentationThemeVariableColor(withWallpaper: .clear, withoutWallpaper: customTheme.accentColor),
			actionButtonsTextColor: PresentationThemeVariableColor(withWallpaper: UIColor(rgb: 0xFFFFFF), withoutWallpaper: customTheme.accentColor),
			textSelectionColor: customTheme.accentColor.withAlphaComponent(0.3),
			textSelectionKnobColor: customTheme.accentColor),
		outgoing: PresentationThemePartedColors(
			bubble: PresentationThemeBubbleColor(
				withWallpaper: PresentationThemeBubbleColorComponents(
					fill: customTheme.outgoingBubble,
			...
			primaryTextColor: customTheme.outgoingText,
			secondaryTextColor: customTheme.outgoingSecondaryText,
			linkTextColor: customTheme.outgoingText,
			linkHighlightColor: customTheme.outgoingSecondaryText,
			scamColor: customTheme.outgoingText,
			textHighlightColor: UIColor(rgb: 0xffc738),
			accentTextColor: customTheme.accentColor,
			accentControlColor: customTheme.accentColor,
			accentControlDisabledColor: customTheme.accentColor.withAlphaComponent(0.5),
			mediaActiveControlColor: customTheme.outgoingText,
			mediaInactiveControlColor: customTheme.outgoingText.withAlphaComponent(0.65),
			mediaControlInnerBackgroundColor: .clear,
			pendingActivityColor: customTheme.outgoingText.withAlphaComponent(0.65),
			fileTitleColor: UIColor(rgb: 0xFFFFFF),
			fileDescriptionColor:customTheme.outgoingText.withAlphaComponent(0.65),
			fileDurationColor: customTheme.outgoingText.withAlphaComponent(0.65),
			mediaPlaceholderColor: UIColor(rgb: 0x0077d9),
			polls: PresentationThemeChatBubblePolls(
				radioButton: customTheme.outgoingText.withAlphaComponent(0.65),
				radioProgress: UIColor(rgb: 0xFFFFFF),
				highlight: UIColor(rgb: 0xFFFFFF, alpha: 0.12),
				separator: UIColor(rgb: 0xFFFFFF, alpha: 0.65),
				bar: UIColor(rgb: 0xFFFFFF),
				barIconForeground: .clear,
				barPositive: UIColor(rgb: 0xFFFFFF),
				barNegative: UIColor(rgb: 0xFFFFFF)
			),
			actionButtonsFillColor: PresentationThemeVariableColor(withWallpaper: serviceBackgroundColor, withoutWallpaper: UIColor(rgb: 0xFFFFFF, alpha: 0.8)),
			actionButtonsStrokeColor: PresentationThemeVariableColor(withWallpaper: .clear, withoutWallpaper: customTheme.accentColor),
			actionButtonsTextColor: PresentationThemeVariableColor(withWallpaper: UIColor(rgb: 0xFFFFFF), withoutWallpaper: customTheme.accentColor),
			textSelectionColor: customTheme.outgoingText.withAlphaComponent(0.2),
			textSelectionKnobColor:  customTheme.outgoingText
		),
		freeform: PresentationThemeBubbleColor(
			withWallpaper: PresentationThemeBubbleColorComponents(
		...
		shareButtonForegroundColor: PresentationThemeVariableColor(withWallpaper: UIColor(rgb: 0xFFFFFF), withoutWallpaper: customTheme.accentColor),
		mediaOverlayControlColors: PresentationThemeFillForeground(fillColor: UIColor(rgb: 0x000000, alpha: 0.45), foregroundColor: UIColor(rgb: 0xFFFFFF)),
		selectionControlColors: PresentationThemeFillStrokeForeground(fillColor: customTheme.accentColor, strokeColor: UIColor(rgb: 0xc7c7cc), foregroundColor: UIColor(rgb: 0xFFFFFF)),
		deliveryFailedColors: PresentationThemeFillForeground(fillColor: UIColor(rgb: 0xff3b30), foregroundColor: UIColor(rgb: 0xFFFFFF)),
	...
	let serviceMessage = PresentationThemeServiceMessage(
		...
		// TODO: Ton - Chat: Date header color
		dateTextColor: PresentationThemeVariableColor(
	...
	let serviceMessageDay = PresentationThemeServiceMessage(
		...
		// TODO: Ton - Chat: Date header color
		dateTextColor: PresentationThemeVariableColor(
			withWallpaper: UIColor(rgb: 0xFFFFFF),
			withoutWallpaper: UIColor(rgb: 0x8d8e93)
		)
	)
	
	let inputPanelMediaRecordingControl = PresentationThemeChatInputPanelMediaRecordingControl(
		buttonColor: customTheme.accentColor,
		micLevelColor: customTheme.accentColor.withAlphaComponent(0.2),
		activeIconColor: UIColor(rgb: 0xFFFFFF)
	)
	
	let inputPanel = PresentationThemeChatInputPanel(
		panelBackgroundColor: customTheme.blurredBackgroundColor,
		panelBackgroundColorNoWallpaper: customTheme.blurredBackgroundColor,
		panelSeparatorColor: UIColor(rgb: 0xbec2c6),
		panelControlAccentColor: customTheme.accentColor,
		panelControlColor: UIColor(rgb: 0x000000),
		...
		actionControlFillColor: customTheme.accentColor,
	...
	let historyNavigation = PresentationThemeChatHistoryNavigation(
		...
		badgeBackgroundColor: customTheme.accentColor,
		badgeStrokeColor: customTheme.accentColor,
		badgeTextColor: UIColor(rgb: 0xFFFFFF)
	)
	...
	let actionSheet = PresentationThemeActionSheet(
		...
		opaqueItemHighlightedBackgroundColor: UIColor(rgb: 0xFFFFFF),
		itemHighlightedBackgroundColor: UIColor(white: 0.9, alpha: 0.7),
		opaqueItemSeparatorColor: UIColor(rgb: 0xFFFFFF),
		standardActionTextColor: customTheme.accentColor,
		...
		controlAccentColor: customTheme.accentColor,
	...
	let contextMenu = PresentationThemeContextMenu(
		dimColor: customTheme.dimColor,
		backgroundColor: UIColor(rgb: 0xf9f9f9, alpha: 1),
		...
		badgeFillColor: customTheme.accentColor,
