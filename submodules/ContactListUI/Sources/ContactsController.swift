    ...
    override public func loadDisplayNode() {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] state in
            ...
            if let state, state.selectedPeerIndices.count > 0 {
                toolbar = Toolbar(leftAction: nil, rightAction: nil,
								  middleAction: ToolbarAction(title: self.presentationData.strings.ContactList_DeleteConfirmation(Int32(state.selectedPeerIndices.count)), isEnabled: true, color: .custom(self.presentationData.theme.actionSheet.destructiveActionTextColor)))
            } else {
                toolbar = nil
            }
