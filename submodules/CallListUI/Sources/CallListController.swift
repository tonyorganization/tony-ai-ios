...
public final class CallListController: TelegramBaseController {
    ...
    public init(context: AccountContext, mode: CallListControllerMode) {
        ...
        |> deliverOnMainQueue).startStrict(next: { [weak self] presentationData in
            if let strongSelf = self {
				let previousData = strongSelf.presentationData
                let previousTheme = previousData.theme
                let previousStrings = previousData.strings
    ...
    private func updateThemeAndStrings() {
		...
		if let navigationBar = self.navigationBar {
			navigationBar.updatePresentationData(NavigationBarPresentationData(presentationData: self.presentationData))
