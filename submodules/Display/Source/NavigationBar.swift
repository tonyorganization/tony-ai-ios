...
public final class NavigationBarTheme {
	...
	public func withUpdatedBackgroundColor(_ color: UIColor) -> NavigationBarTheme {
		return NavigationBarTheme(
			...
			disabledButtonColor: self.disabledButtonColor,
			primaryTextColor: self.primaryTextColor,
			backgroundColor: color,
			opaqueBackgroundColor: self.opaqueBackgroundColor,
			enableBackgroundBlur: true,
			separatorColor: self.separatorColor,
			badgeBackgroundColor: self.badgeBackgroundColor,
			badgeStrokeColor: self.badgeStrokeColor,
			badgeTextColor: self.badgeTextColor
		)
	}
	...
	public func withUpdatedSeparatorColor(_ color: UIColor) -> NavigationBarTheme {
		return NavigationBarTheme(
			...
			opaqueBackgroundColor: self.opaqueBackgroundColor,
			enableBackgroundBlur: self.enableBackgroundBlur,
...
public final class NavigationBackgroundNode: ASDisplayNode {
	...
	public init(color: UIColor, enableBlur: Bool = true, enableSaturation: Bool = true, customBlurRadius: CGFloat? = nil) {
		// TODO: Ton - NavBar: NavigationBackgroundNode adapt navigation bar color
		self._color = color
	...
	public override func didLoad() {
		...
		if self.scheduledUpdate {
			...
			self.updateBackgroundBlur(forceKeepBlur: true)
		}
	}
	...
	private func updateBackgroundBlur(forceKeepBlur: Bool) {
		// TODO: Ton - NavBar: NavigationBackgroundNode adap blur
	...
	public func updateColor(
		color: UIColor,
		enableBlur: Bool? = nil,
		enableSaturation: Bool? = nil,
		forceKeepBlur: Bool = false,
		...
		if self._color.isEqual(UIColor(rgb: 0x060A16)) {
			print("Update dart theme")
		} else if self._color.isEqual(UIColor(rgb: 0xDFE8F4)){
			print("Update light theme")
		} else {
			print("Update unknown theme")
		}
		// TODO: Ton - NavBar
		self.backgroundNode.backgroundColor = self._color
		//if sharedIsReduceTransparencyEnabled {
		//	transition.updateBackgroundColor(node: self.backgroundNode, color: self._color.withAlphaComponent(0.9))
		//} else {
...
open class BlurredBackgroundView: UIView {
	...
	public init(color: UIColor?, enableBlur: Bool = true, customBlurRadius: CGFloat? = nil) {
		// TODO: Ton - NavBar: BlurredBackgroundView adapt navigation bar color
		self._color = color
		self.enableBlur = enableBlur
		...
		self.blurBackgroundImage = UIImageView()
		self.blurBackgroundImage.contentMode = .top
		self.blurBackgroundImage.clipsToBounds = true
		//self.blurBackgroundImage.image = UIImage(named: "Ton/Background13")
		
		super.init(frame: CGRect())
		self.backgroundView.backgroundColor = color
		self.addSubview(self.backgroundView)
		self.addSubview(self.blurBackgroundImage)
	...
	private func updateBackgroundBlur(forceKeepBlur: Bool) {
		...
		// TODO: Ton - NavBar: BlurredBackgroundView dapt blur
	...
	public func updateColor(color: UIColor, enableBlur: Bool? = nil, forceKeepBlur: Bool = false, transition: ContainedViewLayoutTransition) {
		...
		// TODO: Ton - Background: adapt navigation bar color
		self._color = color
		self.enableBlur = effectiveEnableBlur
		if let _color = self._color {
			if sharedIsReduceTransparencyEnabled {
				transition.updateBackgroundColor(layer: self.backgroundView.layer, color: _color.withAlphaComponent(0.9))
			} else {
				transition.updateBackgroundColor(layer: self.backgroundView.layer, color: _color)
			}
		}
		self.updateBackgroundBlur(forceKeepBlur: forceKeepBlur)
	...
	public func update(size: CGSize, cornerRadius: CGFloat = 0.0, maskedCorners: CACornerMask = [.layerMaxXMaxYCorner, .layerMaxXMinYCorner, .layerMinXMaxYCorner, .layerMinXMinYCorner], transition: ContainedViewLayoutTransition) {
		...
		// TODO: Ton - NavBar: BlurredBackgroundView adapt blur
		transition.updateFrame(view: self.blurBackgroundImage, frame: contentFrame, beginWithCurrentState: true)
		self.blurBackgroundImage.frame = contentFrame
		...
		if #available(iOS 11.0, *) {
			...
			//self.blurBackgroundImage.layer.maskedCorners = maskedCorners
		}
	...
	public func update(size: CGSize, cornerRadius: CGFloat = 0.0, animator: ControlledTransitionAnimator) {
		...
		// TODO: Ton - NavBar: BlurredBackgroundView adapt blur
		animator.updateFrame(layer: self.blurBackgroundImage.layer, frame: contentFrame, completion: nil)
		self.blurBackgroundImage.frame = self.bounds
	...
	// TODO: Ton - NavBar: BlurredBackgroundView set background image
	private let blurBackgroundImage : UIImageView
	
	public func updateImage(imageName: String) {
		self.blurBackgroundImage.contentMode = .top
		self.blurBackgroundImage.clipsToBounds = true
		//self.blurBackgroundImage.image = UIImage(named: imageName)
		
	}
	
	// TODO: Ton - NavBar: BlurredBackgroundView sync background color
	public func syncBackground(){
		//self.updateImage(imageName: "Ton/Background1")
		self.backgroundView.backgroundColor = self._color
	}
...
open class NavigationBar: ASDisplayNode {
		...
		didSet {
			if let title = self.title {
				...
				self.titleNode.attributedText = NSAttributedString(
					string: sTitle,
					font: NavigationBar.titleFont,
					textColor: self.presentationData.theme.primaryTextColor
				)
	...
	public init(presentationData: NavigationBarPresentationData) {
		...
		let backgroundColor = self.presentationData.theme.backgroundColor
		self.backgroundNode = NavigationBackgroundNode(
			color: backgroundColor,
			enableBlur: self.presentationData.theme.enableBackgroundBlur
		)
		...
		self.navBackgroundImage = ASImageNode()
		
		
		super.init()
		
		self.addSubnode(self.backgroundNode)
		self.addSubnode(self.navBackgroundImage)
	...
	public func updateBackgroundAlpha(_ alpha: CGFloat, transition: ContainedViewLayoutTransition) {
		...
		if alpha <= 0.0{
			print("NavigationBar updateBackgroundAlpha with alpha <= 0.0")
		}
	...
	public func updatePresentationData(_ presentationData: NavigationBarPresentationData, transition: ContainedViewLayoutTransition = .immediate) {
		...
		if presentationData.theme !== self.presentationData.theme || presentationData.strings !== self.presentationData.strings {
			...
			self.backgroundNode.updateColor(
				color: presentationData.theme.backgroundColor,
	...
	override open func layout() {
		...
		if let validLayout = self.validLayout, self.requestedLayout {
			...
			self.navBackgroundImage.frame = self.bounds
	...
	func updateLayout(size: CGSize, defaultHeight: CGFloat, additionalTopHeight: CGFloat, additionalContentHeight: CGFloat, additionalBackgroundHeight: CGFloat, additionalCutout: CGSize?, leftInset: CGFloat, rightInset: CGFloat, appearsHidden: Bool, isLandscape: Bool, transition: ContainedViewLayoutTransition) {
		...
		if self.backgroundNode.frame != backgroundFrame {
			...
			// TODO: Ton - NavigationBar: background image frame
			transition.updateFrame(node: self.navBackgroundImage, frame: backgroundFrame)
	...
	public func setHidden(_ hidden: Bool, animated: Bool) {
		if let contentNode = self.contentNode, case .replacement = contentNode.mode {
			...
			if hidden {
				print("NavigationBar setHidden to true" )
			}
	...
	// TODO: Ton - NavigationBar: set background image
	private var navBackgroundImage: ASImageNode
	
	public func updateImage(imageName: String) {
		self.navBackgroundImage.contentMode = .top
		self.navBackgroundImage.clipsToBounds = true
		self.navBackgroundImage.image = UIImage(named: imageName)
	}
	...
	// TODO: Ton - NavBar: background image adapt
	public func syncBackground() {
		//self.updateImage(imageName: "Ton/Background1")
	}
	
	public func hideSeparator(){
		self.stripeNode.backgroundColor = UIColor(white: 0, alpha: 0)
	}
