...
public class TitleTextFieldNode: ASControlNode {
	...
	init(title: String) {
		// Prepare the UITextField wrapper node
		self.textFieldNode = ASDisplayNode(viewBlock: { [weak textField] in
			return textField ?? UITextField()
		})
		...
		self.textFieldNode.style.height = ASDimension(unit: .points, value: 38)
	}
