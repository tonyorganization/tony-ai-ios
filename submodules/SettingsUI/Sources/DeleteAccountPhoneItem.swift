        ...
        let processNumberChange: (String) -> Bool = { [weak self] number in
            ...
            if let (country, _) = AuthorizationSequenceCountrySelectionController.lookupCountryIdByNumber(number, preferredCountries: strongSelf.preferredCountryIdForCode) {
                ...
                if let mask = AuthorizationSequenceCountrySelectionController.lookupPatternByNumber(number, preferredCountries: strongSelf.preferredCountryIdForCode).flatMap({ NSAttributedString(string: $0, font: maskFont, textColor: item.theme.list.itemPlaceholderTextColor) }) {
                    ...
                    strongSelf.phoneInputNode.phoneNumberNode.textField.attributedPlaceholder = NSAttributedString(string: item.strings.Login_PhonePlaceholder, font: Font.regular(17.0), textColor: item.theme.list.itemPlaceholderTextColor)
                }
                return true
            } else {
                return false
        ...
        return { item, params, neighbors in
            ...
            return (layout, { [weak self] in
                if let strongSelf = self {
                    ...
                    strongSelf.phoneInputNode.dialingTextNode.textField.textColor = item.theme.list.itemPrimaryTextColor
                    strongSelf.phoneInputNode.dialingTextNode.textField.keyboardAppearance = item.theme.rootController.keyboardColor.keyboardAppearance
                    strongSelf.phoneInputNode.dialingTextNode.textField.tintColor = item.theme.list.itemAccentColor
                    strongSelf.phoneInputNode.phoneNumberNode.textField.textColor = item.theme.list.itemPrimaryTextColor
                    strongSelf.phoneInputNode.phoneNumberNode.textField.keyboardAppearance = item.theme.rootController.keyboardColor.keyboardAppearance
                    strongSelf.phoneInputNode.phoneNumberNode.textField.tintColor = item.theme.list.itemAccentColor
                    ...
                    strongSelf.phoneInputNode.dialingTextNode.frame = countryCodeFrame.offsetBy(dx: -phoneInputFrame.minX, dy: -phoneInputFrame.minY)
                    strongSelf.phoneInputNode.phoneNumberNode.frame = numberFrame.offsetBy(dx: -phoneInputFrame.minX, dy: -phoneInputFrame.minY)
