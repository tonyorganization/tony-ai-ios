...
final class ChangePhoneNumberControllerNode: ASDisplayNode {
    ...
    init(presentationData: PresentationData) {
        ...
        self.noticeNode.attributedText = NSAttributedString(
			string: self.presentationData.strings.ChangePhoneNumberNumber_Help,
        ...
        self.phoneInputNode.dialingTextNode.textField.textColor = self.presentationData.theme.list.itemPrimaryTextColor
        self.phoneInputNode.dialingTextNode.textField.keyboardAppearance = self.presentationData.theme.rootController.keyboardColor.keyboardAppearance
        self.phoneInputNode.dialingTextNode.textField.tintColor = self.presentationData.theme.list.itemAccentColor
        self.phoneInputNode.phoneNumberNode.textField.textColor = self.presentationData.theme.list.itemPrimaryTextColor
        self.phoneInputNode.phoneNumberNode.textField.keyboardAppearance = self.presentationData.theme.rootController.keyboardColor.keyboardAppearance
        self.phoneInputNode.phoneNumberNode.textField.tintColor = self.presentationData.theme.list.itemAccentColor
        ...
        self.phoneInputNode.phoneNumberNode.textField.attributedPlaceholder = NSAttributedString(string: self.presentationData.strings.Login_PhonePlaceholder, font: Font.regular(17.0), textColor: self.presentationData.theme.list.itemPlaceholderTextColor)
        ...
        let processNumberChange: (String) -> Bool = { [weak self] number in
            ...
            if let (country, _) = AuthorizationSequenceCountrySelectionController.lookupCountryIdByNumber(number, preferredCountries: strongSelf.preferredCountryIdForCode) {
                ...
                if let mask = AuthorizationSequenceCountrySelectionController.lookupPatternByNumber(number, preferredCountries: strongSelf.preferredCountryIdForCode).flatMap({ NSAttributedString(string: $0, font: maskFont, textColor: strongSelf.presentationData.theme.list.itemPlaceholderTextColor) }) {
                    strongSelf.phoneInputNode.phoneNumberNode.textField.attributedPlaceholder = nil
                    strongSelf.phoneInputNode.mask = mask
                } else {
                    strongSelf.phoneInputNode.mask = nil
                    strongSelf.phoneInputNode.phoneNumberNode.textField.attributedPlaceholder = NSAttributedString(string: strongSelf.presentationData.strings.Login_PhonePlaceholder, font: Font.regular(17.0), textColor: strongSelf.presentationData.theme.list.itemPlaceholderTextColor)
                }
                return true
            } else {
                return false
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        transition.updateFrame(node: self.phoneInputNode.dialingTextNode, frame: countryCodeFrame.offsetBy(dx: -phoneInputFrame.minX, dy: -phoneInputFrame.minY))
        transition.updateFrame(node: self.phoneInputNode.phoneNumberNode, frame: numberFrame.offsetBy(dx: -phoneInputFrame.minX, dy: -phoneInputFrame.minY))
    ...
    func activateInput() {
        self.phoneInputNode.phoneNumberNode.textField.becomeFirstResponder()
    }
    
    func animateError() {
        self.phoneInputNode.dialingTextNode.layer.addShakeAnimation()
        self.phoneInputNode.phoneNumberNode.layer.addShakeAnimation()
    }
