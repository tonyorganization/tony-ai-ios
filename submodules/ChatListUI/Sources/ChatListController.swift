...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
    ...
    public init(context: AccountContext, location: ChatListControllerLocation, controlsHistoryPreload: Bool, hideNetworkActivityStatus: Bool = false, previewing: Bool = false, enableDebugActions: Bool) {
        ...
        if case .chatList(.root) = self.location {
            self.chatListDisplayNode.mainContainerNode.currentItemFilterUpdated = { [weak self] filter, fraction, transition, force in
				...
				// TODO: Ton - ChatFolder: update filter list
                strongSelf.tabContainerNode.update(
					size: CGSize(width: layout.size.width, height: 46.0),
					sideInset: layout.safeInsets.left,
					filters: tabContainerData.0,
					selectedFilter: filter,
					isReordering: strongSelf.chatListDisplayNode.isReorderingFilters || (strongSelf.chatListDisplayNode.mainContainerNode.currentItemNode.currentState.editing && !strongSelf.chatListDisplayNode.didBeginSelectingChatsWhileEditing),
					isEditing: strongSelf.chatListDisplayNode.mainContainerNode.currentItemNode.currentState.editing,
					canReorderAllChats: strongSelf.isPremium,
    ...
    private func updateThemeAndStrings() {
        ...
        if let layout = self.validLayout {
			// TODO: Ton - ChatFolder: update filter list
            self.tabContainerNode.update(
				size: CGSize(width: layout.size.width, height: 46.0),
				sideInset: layout.safeInsets.left,
				filters: self.tabContainerData?.0 ?? [],
				selectedFilter: self.chatListDisplayNode.effectiveContainerNode.currentItemFilter,
				isReordering: self.chatListDisplayNode.isReorderingFilters || (self.chatListDisplayNode.effectiveContainerNode.currentItemNode.currentState.editing && !self.chatListDisplayNode.didBeginSelectingChatsWhileEditing),
				isEditing: self.chatListDisplayNode.effectiveContainerNode.currentItemNode.currentState.editing,
				canReorderAllChats: self.isPremium,
				filtersLimit: self.tabContainerData?.2,
				transitionFraction: self.chatListDisplayNode.effectiveContainerNode.transitionFraction,
    ...
    override public func loadDisplayNode() {
        ...
        self.tabContainerNode.tabSelected = { [weak self] id, isDisabled in
			...
			if id == .addItem {
				// TODO: Ton - ChatFolder: onAddItemTap
				strongSelf.onAddItemTap()
				return
			}
            if isDisabled {
                let context = strongSelf.context
...
//        }
    ...
    fileprivate func maybeDisplayStoryTooltip() {
		// TODO: Ton - ChatList: Disable tooltip for now
...
//            })
	...
	private func onAddItemTap(){
		...
		let _ = combineLatest(
			...
			guard let parentController = strongSelf.parent as? TabBarController  else {
				return
			}
			
			let alertLimitFolder = {
				let alertController =  strongSelf.context.sharedContext.simpleTextAlert(
					text: "Limit folder created"
				)
			...
			let controller = chatListFilterPresetController(context: strongSelf.context, currentPreset: nil, updated: { _ in })
    ...
    private func updateLayout(layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        if !skipTabContainerUpdate {
            ...
            self.tabContainerNode.update(
				...
				sideInset: layout.safeInsets.left,
				filters: self.tabContainerData?.0 ?? [],
				selectedFilter: self.chatListDisplayNode.mainContainerNode.currentItemFilter,
				isReordering: self.chatListDisplayNode.isReorderingFilters || (self.chatListDisplayNode.effectiveContainerNode.currentItemNode.currentState.editing && !self.chatListDisplayNode.didBeginSelectingChatsWhileEditing),
				isEditing: self.chatListDisplayNode.effectiveContainerNode.currentItemNode.currentState.editing,
				canReorderAllChats: self.isPremium,
				filtersLimit: self.tabContainerData?.2,
    ...
    private func reloadFilters(firstUpdate: (() -> Void)? = nil) {
        ...
        |> deliverOnMainQueue).startStrict(next: { [weak self] countAndFilterItems, peerView, limits in
			...
			if !items.isEmpty {
				// TODO: Ton - ChatFolder: append add button when filter node visible
				filterItems.append(.addItem(enable: true))
			}
...
private final class ChatListLocationContext {
    ...
    private func updateChatList(
		...
		// TODO: Ton - ChatList: disable show story tooltip for now
        //Queue.mainQueue().after(1.0, { [weak self] in
        //    guard let self else {
        //        return
        //    }
