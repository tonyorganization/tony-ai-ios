...
// TODO: Ton - ChatFolder: List Item Node
private final class ItemNode: ASDisplayNode {
	...
	private let filter : ChatListFilterTabEntry
	init(
		context: AccountContext,
		theme: PresentationTheme? = nil,
		filter: ChatListFilterTabEntry,
		pressed: @escaping (Bool) -> Void,
		requestedDeletion: @escaping () -> Void,
		contextGesture: @escaping (ContextExtractedContentContainingNode, ContextGesture, Bool) -> Void
	) {
		...
		// TODO: Ton - ChatFolder: disable background alpha anim
		//self.containerNode.backgroundColor = .red.
		self.iconNode = ASImageNode()
		self.extractedBackgroundNode = ASImageNode()
		//self.extractedBackgroundNode.alpha = 0.0
		//self.extractedBackgroundNode.backgroundColor = .blue
		//self.extractedBackgroundNode.cornerRadius = 18.0
		//self.extractedBackgroundNode.alpha = 1.0
		...
		self.titleNode.insets = UIEdgeInsets(top: titleInset, left: 4.0, bottom: titleInset, right: 4.0)
		...
		self.titleActiveNode.insets = UIEdgeInsets(top: titleInset, left: 4.0, bottom: titleInset, right: 4.0)
		...
		self.shortTitleNode.insets = UIEdgeInsets(top: titleInset, left: 4.0, bottom: titleInset, right: 4.0)
		...
		self.shortTitleActiveNode.insets = UIEdgeInsets(top: titleInset, left: 4.0, bottom: titleInset, right: 4.0)
		self.shortTitleActiveNode.alpha = 0.0
		...
		super.init()
		//self.extractedContainerNode.contentNode.addSubnode(self.backgroundNode)
		...
		// TODO: Ton - ChatFolder: always show item background
		if let theme = self.theme {
			//self.extractedBackgroundNode.image = generateStretchableFilledCircleImage(
			//	diameter: 26.0,
			//	color: theme.custom.unselectButton
			//)
			self.extractedBackgroundNode.backgroundColor = theme.custom.unselectButton
		}
		//        self.extractedContainerNode.willUpdateIsExtractedToContextPreview = { [weak _ ] isExtracted, transition in
		//            guard let strongSelf = self else {
		//                return
		//            }
		//			// TODO: Ton - ChatFolder: disable show and hide item background
		//            if isExtracted, let theme = strongSelf.theme {
		//                strongSelf.extractedBackgroundNode.image = generateStretchableFilledCircleImage(
		//					diameter: 26.0,
		//					color: theme.custom.unselectButton
		//				)
		//            }
	...
	// TODO: Ton - ChatFolder: update item on tapped
	func updateText(
		strings: PresentationStrings,
		iconName: String?,
		title: ChatFolderTitle,
		shortTitle: ChatFolderTitle,
		unreadCount: Int,
		unreadHasUnmuted: Bool,
		isNoFilter: Bool,
		selectionFraction: CGFloat,
		isEditing: Bool,
		isReordering: Bool,
		canReorderAllChats: Bool,
		isDisabled: Bool,
		...
		let bagdeSize = 18.0
		// TODO: Ton - ChatFolder: badge color
		if presentationData.theme.custom.isDark {
			self.badgeBackgroundActiveNode.image = generateStretchableFilledCircleImage(
				diameter: bagdeSize,
				color: .white
			)
			self.badgeBackgroundInactiveNode.image = generateStretchableFilledCircleImage(
				diameter: bagdeSize,
				color: self.theme?.custom.accentColor
			...
			self.badgeBackgroundInactiveNode.image = generateStretchableFilledCircleImage(
				diameter: bagdeSize,
				color: self.theme?.custom.accent2Color
		...
		let isSelected = selectionFraction == 1.0
		if isSelected {
			self.buttonNode.accessibilityTraits = [.button, .selected]
		} else {
			self.buttonNode.accessibilityTraits = [.button]
		}
		// TODO: Ton - ChatFolder: rounded item background
		if isSelected {
			self.extractedBackgroundNode.backgroundColor = presentationData.theme.custom.selectedButton
		} else {
			self.extractedBackgroundNode.backgroundColor = presentationData.theme.custom.unselectButton
		}
		
		if let image = iconName {
			if isSelected {
				self.iconNode.image = generateTintedImage(
					image: UIImage(bundleImageName: image),
					color: presentationData.theme.custom.selectedButtonText
				)
			} else {
				self.iconNode.image = generateTintedImage(
					image: UIImage(bundleImageName: image),
					color: presentationData.theme.custom.unselectButtonText
				)
			}
		...
		if isSelected {
			self.badgeBackgroundActiveNode.alpha = 1.0
			self.badgeBackgroundInactiveNode.alpha = 0.0
			//transition.updateAlpha(node: self.badgeBackgroundActiveNode, alpha: 1)
			//transition.updateAlpha(node: self.badgeBackgroundInactiveNode, alpha: 0)

		} else {
			//transition.updateAlpha(node: self.badgeBackgroundActiveNode, alpha: 0)
			//transition.updateAlpha(node: self.badgeBackgroundInactiveNode, alpha: 1)
			self.badgeBackgroundActiveNode.alpha = 0.0
		...
		if themeUpdated || titleUpdated {
			self.titleNode.attributedText = title.attributedString(
				font: Font.medium(14.0),
				textColor: presentationData.theme.custom.unselectButtonText
			)
			self.titleActiveNode.attributedText = title.attributedString(
				font: Font.medium(14.0),
				textColor: presentationData.theme.custom.selectedButtonText
			)
			
			self.shortTitleNode.attributedText = shortTitle.attributedString(
				font: Font.medium(14.0),
				textColor: presentationData.theme.list.itemSecondaryTextColor
			)
			self.shortTitleActiveNode.attributedText = shortTitle.attributedString(
				font: Font.medium(14.0),
				textColor: presentationData.theme.list.itemAccentColor
			)
		}
		
		if unreadCount != 0 {
			if themeUpdated || unreadCountUpdated || self.badgeTextNode.attributedText == nil {
				self.badgeTextNode.attributedText = NSAttributedString(
					string: "\(unreadCount)",
	...
	func updateLayout(
		height: CGFloat,
		transition: ContainedViewLayoutTransition
	) -> (width: CGFloat, shortWidth: CGFloat) {
		let iconSize : CGFloat
		if self.filter.id == .all || self.filter.id == .addItem {
			iconSize = 24
		} else {
			iconSize = 0
		}
		...
		let _ = self.titleActiveNode.updateLayout(
			CGSize(
				width: 160.0,
				height: .greatestFiniteMagnitude
			)
		...
		let titleFrame = CGRect(
			origin: CGPoint(
				x: -self.titleNode.insets.left + iconSize + 2,
		...
		let shortTitleSize = self.shortTitleNode.updateLayout(
			CGSize(
				width: 160.0,
				height: .greatestFiniteMagnitude
			)
		...
		let shortTitleFrame = CGRect(
			origin: CGPoint(
				x: -self.shortTitleNode.insets.left + iconSize,
				y: floor((height - shortTitleSize.height) / 2.0)
			),
			size: shortTitleSize
		)
		...
		return (
			width + iconSize + 2,
			shortTitleSize.width - self.shortTitleNode.insets.left - self.shortTitleNode.insets.right + iconSize
		)
	}
	...
	func updateArea(
		...
		self.buttonNode.frame = CGRect(
			...
			size: CGSize(
				width: size.width + sideInset * 2.0,
				height: size.height
			)
		)
		...
		self.extractedContainerNode.contentRect = CGRect(
			origin: CGPoint(
				x: self.extractedBackgroundNode.frame.minX,
				y: 0.0
			),
			size: CGSize(
				width:self.extractedBackgroundNode.frame.width,
				height: size.height
			)
		)
		...
		// TODO: Ton - ChatFolder: inset and margin
		let extractedBackgroundHeight: CGFloat = 34.0
		let extractedBackgroundInset: CGFloat = 10.0
		self.extractedBackgroundNode.cornerRadius = extractedBackgroundHeight/2
		self.extractedBackgroundNode.frame = CGRect(
			origin: CGPoint(
				x: -extractedBackgroundInset,
				y: floor((size.height - extractedBackgroundHeight) / 2.0)),
			size: CGSize(
				width: size.width + extractedBackgroundInset * 2.0,
				height: extractedBackgroundHeight
			)
		)
	}
...
public final class ChatListFilterTabContainerNode: ASDisplayNode {
	...
	public func update(
		...
		presentationData: PresentationData,
		transition proposedTransition: ContainedViewLayoutTransition
	) {
		...
		// TODO: Ton - ChatFolder: skip add item tab from focus
		...
		for i in 0 ..< reorderedFilters.count {
			...
			if let current = self.itemNodes[filter.id] {
				...
				wasAdded = true
				// TODO: Ton - ChatFolder, init ItemNode
				itemNode = ItemNode(
					context: self.context,
					theme: presentationData.theme,
					filter: filter,
					pressed: { [weak self] disabled in
			...
			case let .all(count):
				...
				iconName = "Ton/IconUser"
				isNoFilter = true
			case let .addItem(enable):
				unreadCount = 0
				unreadHasUnmuted = enable
				iconName = "Ton/IconAddFolder"
				isNoFilter = enable
			...
			// TODO: Ton - ChatFolder: update item node content
			itemNode.updateText(
				strings: presentationData.strings,
				iconName: iconName,
				title: filter.title(strings: presentationData.strings),
				shortTitle: i == 0 ? filter.shortTitle(strings: presentationData.strings) : filter.title(strings: presentationData.strings),
				unreadCount: unreadCount,
				unreadHasUnmuted: unreadHasUnmuted,
				isNoFilter: isNoFilter,
				selectionFraction: selectionFraction,
				isEditing: isEditing,
				isReordering: isReordering,
				canReorderAllChats: canReorderAllChats,
		...
		for filter in reorderedFilters {
			...
			// TODO: Ton - ChatFolder: update item node updateLayout
			let (paneNodeWidth, paneNodeShortWidth) = itemNode.updateLayout(
				height: size.height,
				transition: itemNodeTransition
			)
		...
		for i in 0 ..< tabSizes.count {
			...
			// TODO: Ton - ChatFolder: update item node area
			paneNode.updateArea(
				size: paneFrame.size,
				sideInset: minSpacing / 2.0,
