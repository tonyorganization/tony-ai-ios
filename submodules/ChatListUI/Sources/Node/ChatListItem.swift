...
private final class ChatListItemTagListComponent: Component {
    ...
    private final class ItemView: UIView {
        ...
        func update(
			...
			// TODO: Ton - Background: clear item color
...
public class ChatListItemNode: ItemListRevealOptionsItemNode {
                ...
                return (CGSize(width: ((iconId == nil && iconColor == nil && threadPeer == nil) ? 1.0 : 18.0) + 2.0 + topicTitleLayout.0.size.width, height: topicTitleLayout.0.size.height), {
                    ...
                    if let titleTopicIconView, let titleTopicIconComponent {
                        ...
                        if threadPeer.smallProfileImage != nil {
                            titleTopicAvatarNode.setPeerV2(
								context: context,
								theme: theme,
								peer: threadPeer,
								overrideImage: nil,
								emptyColor: theme.list.mediaPlaceholderColor,
								// TODO: Ton - ChatList: Edit avatar clipStyle
								clipStyle: .roundedRect,
								//clipStyle: .round,
								synchronousLoad: false,
								displayDimensions: iconSize
							)
                        } else {
                            titleTopicAvatarNode.setPeer(
								context: context,
								theme: theme,
								peer: threadPeer,
								overrideImage: nil,
    ...
    func setupItem(item: ChatListItem, synchronousLoads: Bool) {
        ...
        if let peer = peer {
            ...
            var avatarClipStyle: AvatarNodeClipStyle  = .roundedRect
            ...
            if peer.smallProfileImage != nil && overrideImage == nil {
                self.avatarNode.setPeerV2(
					...
					clipStyle: avatarClipStyle,
					synchronousLoad: synchronousLoads,
					displayDimensions: CGSize(width: avatarDiameter, height: avatarDiameter)
				)
            } else {
                self.avatarNode.setPeer(
					...
					clipStyle: avatarClipStyle,
					synchronousLoad: synchronousLoads,
					displayDimensions: CGSize(width: 60.0, height: 60.0)
				)
            }
