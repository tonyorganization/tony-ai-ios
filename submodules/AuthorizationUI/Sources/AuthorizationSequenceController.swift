...
public final class AuthorizationSequenceController: NavigationController, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    public init(sharedContext: SharedAccountContext, account: UnauthorizedAccount, otherAccountPhoneNumbers: ((String, AccountRecordId, Bool)?, [(String, AccountRecordId, Bool)]), presentationData: PresentationData, openUrl: @escaping (String) -> Void, apiId: Int32, apiHash: String, authorizationCompleted: @escaping () -> Void) {
        ...
        |> map { state -> InnerState in
			...
			// TODO: Ton - Intro: made intro always show when user unauthorized
			//if case .authorized = state {
			//	return .authorized
			//} else {
			//	return .state(.empty)
	...
	// TODO: Ton - Auth: setTestEnvironment
	public func setTestEnvironment(){
		if let controller = self.topViewController as? ViewController {
			let alertController =  self.sharedContext.confirmTextAlert(
				text: "Do you want login to test environment",
				action: {
					self.apiId = 17349
					self.apiHash = "344583e45741c457fe1862106095a5eb"
					let _ = self.sharedContext.accountManager.transaction({ transaction -> Void in
						let _ = transaction.createAuth([.environment(AccountEnvironmentAttribute(environment: .test))])
					}).start(completed: {
						let useTestNetwork =
						UserDefaults.standard.bool(forKey: "useTestNetwork")
						print("Telegram Network:", useTestNetwork ? "TEST DC" : "PROD DC")
					})
				}
			)
	...
	public func setProdEnvironment(){
		if let controller = self.topViewController as? ViewController {
			let alertController =  self.sharedContext.confirmTextAlert(
				text: "Do you want login to prod environment",
				action: {
					let _ = self.sharedContext.accountManager.transaction({ transaction -> Void in
	...
	private var isTestPhoneNumber: Bool = false
	
	// TODO: Ton - Auth: splashController
            ...
            controller.nextPressed = { [weak self] strings in
                if let strongSelf = self {
					...
					let state = UnauthorizedAccountState(
						...
						contents: .phoneEntry(countryCode: countryCode, number: "")
					)
					let _ = strongSelf.engine.auth.setState(state: state).startStandalone()
					// TODO: Ton - Intro: Start messsaging press on intro state
					var controllers: [ViewController] = []
					controllers.append(strongSelf.phoneEntryController(countryCode: countryCode, number: "", splashController: nil))
    ...
    private func phoneEntryController(countryCode: Int32, number: String, splashController: AuthorizationSequenceSplashController?) -> AuthorizationSequencePhoneEntryController {
        ...
        if let currentController = currentController {
            ...
            controller = AuthorizationSequencePhoneEntryController(
				...
				otherAccountPhoneNumbers: self.otherAccountPhoneNumbers,
            ...
            controller.loginWithNumber = { [weak self, weak controller] number, syncContacts in
                ...
                let _ = (authorizationPushConfiguration
					...
					if let strongSelf = self {
						...
						// TODO: Ton - Auth: Add test phone number
//						if number == "+84935491290" || strongSelf.isTestPhoneNumber{
//
//						}
						
						// TODO: Ton - Auth: Add test phone number - end
                        strongSelf.actionDisposable.set((sendAuthorizationCode(
							accountManager: strongSelf.sharedContext.accountManager,
							account: strongSelf.account,
							phoneNumber: number,
							apiId: strongSelf.apiId,
							apiHash: strongSelf.apiHash,
							pushNotificationConfiguration: authorizationPushConfiguration,
							firebaseSecretStream: strongSelf.sharedContext.firebaseSecretStream,
							syncContacts: syncContacts,
							disableAuthTokens: disableAuthTokens,
	...
	// TODO: Ton - Auth: Login by test phone number
	private func loginByTestPhoneNumber(
		controller : AuthorizationSequencePhoneEntryController?,
		countryCode: Int32,
		number: String
	) {
		...
		//	masterDatacenterId: 6
		//)
		let switchAccount = accountManager.transaction { transaction in
    ...
    private func codeEntryController(number: String, phoneCodeHash: String, email: String?, type: SentAuthorizationCodeType, nextType: AuthorizationCodeNextType?, timeout: Int32?, previousCodeType: SentAuthorizationCodeType?, isPrevious: Bool, termsOfService: (UnauthorizedAccountTermsOfService, Bool)?) -> AuthorizationSequenceCodeEntryController {
        ...
        if let currentController = currentController {
            ...
            controller = AuthorizationSequenceCodeEntryController(presentationData: self.presentationData, back: { [weak self] in
                ...
                let _ = strongSelf.engine.auth.setState(
                                         ...
                                         Queue.mainQueue().async {
                                             ...
                                             case .codeExpired:
                                                 ...
                                                 let _ = self.engine.auth.setState(
													...
													isTestingEnvironment: account.testingEnvironment,
													masterDatacenterId: account.masterDatacenterId,
													contents: .empty)).startStandalone()
                                             }
        ...
        controller.requestNextOption = { [weak self, weak controller] in
            if let strongSelf = self {
                ...
                if nextType == nil {
                    ...
                    controller?.inProgress = true
                    strongSelf.actionDisposable.set((resendAuthorizationCode(
						accountManager: strongSelf.sharedContext.accountManager,
						account: strongSelf.account,
						apiId: strongSelf.apiId,
						apiHash: strongSelf.apiHash,
