...
public final class AuthorizationSequenceSplashController: ViewController {
    ...
    init(accountManager: AccountManager<TelegramAccountManagerTypes>, account: UnauthorizedAccount, theme: PresentationTheme) {
		...
		// TODO: Ton: Background image for intro screen
		let backgroundImage : String
		let isDark = theme.custom.isDark
		if isDark {
			backgroundImage = "Ton/IntroBackgroundNight"
		} else {
			backgroundImage = "Ton/IntroBackgroundDay"
		}
		let backgroundColor = theme.custom.backgroundColor
		self.controller = RMIntroViewController(
			backgroundColor: backgroundColor,
			backgroundImage: backgroundImage,
			titleColor: theme.custom.primaryTextColor,
			descriptionColor: theme.custom.secondaryTextColor,
			buttonColor: theme.intro.startButtonColor,
			accentColor: theme.intro.accentTextColor,
			regularDotColor: theme.intro.dotColor,
			highlightedDotColor: theme.list.itemAccentColor,
			suggestedLocalizationSignal: localizationSignal
		)
		...
		self.controller.setTestEnvironment = { [weak self] in
			self?.setTestEnvironment()
		}
		self.controller.setProdEnvironment = { [weak self] in
			self?.setProdEnvironment()
		}
	...
	func setTestEnvironment() {
		if let navController = self.navigationController as? AuthorizationSequenceController {
			navController.setTestEnvironment()
		}
	}
	
	func setProdEnvironment() {
		if let navController = self.navigationController as? AuthorizationSequenceController {
			navController.setProdEnvironment()
		}
	}
    ...
    private func activateLocalization(_ code: String) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] currentCode, suggestedCode in
            ...
            if currentCode == code {
				// TODO: Ton - Verify phone: hide intro headling and description when start messaging pressed
				strongSelf.controller.view.alpha = 0.0
