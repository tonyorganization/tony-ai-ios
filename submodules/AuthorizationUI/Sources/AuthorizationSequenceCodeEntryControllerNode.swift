...
final class AuthorizationSequenceCodeEntryControllerNode: ASDisplayNode, UITextFieldDelegate {
	...
	private let lockImageNode: ASImageNode
    ...
    init(presentationData: PresentationData) {
		...
		// TODO: Ton - Code verify: create lock image
		self.lockImageNode = ASImageNode()
		self.lockImageNode.image = UIImage(named: "Ton/Lock")
		self.lockImageNode.contentMode = .scaleAspectFit
		...
		self.addSubnode(self.lockImageNode)
    ...
    func updateData(number: String, email: String?, codeType: SentAuthorizationCodeType, nextType: AuthorizationCodeNextType?, timeout: Int32?, appleSignInAllowed: Bool, previousCodeType: SentAuthorizationCodeType?, isPrevious: Bool) {
		...
		let currentOptionText = authorizationCurrentOptionText(codeType, phoneNumber: self.phoneNumber, email: self.email, strings: self.strings, primaryColor: self.theme.list.itemPrimaryTextColor, accentColor: self.theme.list.itemAccentColor)
		// TODO: Ton - Code verify: edit Telegram text
//		let copyOptionString = NSMutableAttributedString(attributedString: currentOptionText)
//		var s = copyOptionString.string
//		if let range = s.range(of: "Telegram") {
//			s.replaceSubrange(range, with: "Tongram")
//		}
//		copyOptionString.mutableString.setString(s)
		
        self.currentOptionNode.attributedText = currentOptionText
		//--
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
		...
		insets.top = (layout.statusBarHeight ?? 16.0) + navigationBarHeight
        ...
        if let codeType = self.codeType {
            ...
            case .missedCall:
                self.titleNode.attributedText = NSAttributedString(
					string: self.strings.Login_EnterMissingDigits,
					font: Font.bold(20.0),
					textColor: self.theme.custom.accentColor
				)
            case .email:
                self.titleNode.attributedText = NSAttributedString(
					string: self.strings.Login_EnterCodeEmailTitle,
					font: Font.bold(20.0),
					textColor: self.theme.custom.accentColor
				)
            ...
            case .sms:
                self.titleNode.attributedText = NSAttributedString(
					string: self.strings.Login_EnterCodeSMSTitle,
					font: Font.bold(20.0),
					textColor: self.theme.custom.accentColor
				)
            case .fragment:
                self.titleNode.attributedText = NSAttributedString(
					string: self.strings.Login_EnterCodeFragmentTitle,
					font: Font.bold(20.0),
					textColor: self.theme.custom.accentColor
				)
            ...
            case .word:
                self.titleNode.attributedText = NSAttributedString(
					string: self.strings.Login_EnterWordTitle,
					font: Font.bold(20.0),
					textColor: self.theme.custom.accentColor
				)
            ...
            case .phrase:
                self.titleNode.attributedText = NSAttributedString(
					string: self.strings.Login_EnterPhraseTitle,
					font: Font.bold(20.0),
					textColor: self.theme.custom.accentColor
				)
            ...
            default:
                self.titleNode.attributedText = NSAttributedString(
					string: self.strings.Login_EnterCodeAppNameTitle,
					font: Font.bold(20.0),
					textColor: self.theme.custom.accentColor
				)
            }
		...
		// TODO: Ton - Code verify: hide animation
		...
		let animationSize = CGSize(width: 80, height: 80)
		// TODO: Ton - Verify code: hide animation
...
//        }
		...
		if layout.size.width > 320.0 {
			...
			items.append(AuthorizationLayoutItem(
				node: self.lockImageNode,
				size: animationSize,
				spacingBefore: AuthorizationLayoutItemSpacing(weight: 10.0, maxValue: 10.0),
        ...
        if let codeType = self.codeType {
            ...
            case .missedCall:
                ...
                if self.titleIconNode.image == nil {
					// TODO: Ton - Code verify: edit lock image
					self.titleIconNode.image = UIImage(named: "Ton/Lock")
