...
- (instancetype)initWithBackgroundColor:(UIColor *)backgroundColor backgroundImage:(NSString *)backgroundImage titleColor:(UIColor *)titleColor descriptionColor:(UIColor *)descriptionColor buttonColor:(UIColor *)buttonColor accentColor:(UIColor *)accentColor regularDotColor:(UIColor *)regularDotColor highlightedDotColor:(UIColor *)highlightedDotColor suggestedLocalizationSignal:(SSignal *)suggestedLocalizationSignal
{
	...
	if (self != nil)
	{
		...
		//        headlines = @[ _englishStrings[@"Tour.Title1"],
		//						_englishStrings[@"Tour.Title2"],
		//						_englishStrings[@"Tour.Title3"],
		//						_englishStrings[@"Tour.Title4"],
		//						_englishStrings[@"Tour.Title5"],
		//						_englishStrings[@"Tour.Title6"]
		//		];
		// TODO: Ton - Intro: reduct page size to 5
		headlines = @[ _englishStrings[@"Tour.Title1"],
					   _englishStrings[@"Tour.Title2"],
					   _englishStrings[@"Tour.Title3"],
					   _englishStrings[@"Tour.Title4"],
		...
		_descriptions = @[_englishStrings[@"Tour.Text1"],
						  _englishStrings[@"Tour.Text2"],
						  _englishStrings[@"Tour.Text3"],
						  _englishStrings[@"Tour.Text4"],
						  _englishStrings[@"Tour.Text5"],
						  _englishStrings[@"Tour.Text6"]
		];
		...
		// TODO: Ton - Intro: background
		backgroundImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:backgroundImage]];
		// TODO: Ton - Intro: introImageView init
		introImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Ton/AppIcon"]];
		
		// TODO: Ton - Intro: testButton init
		testButton = [UIButton buttonWithType:UIButtonTypeCustom];
		testButton.backgroundColor = [UIColor clearColor];
		UITapGestureRecognizer *testTap =
		[[UITapGestureRecognizer alloc] initWithTarget:self
												action:@selector(testButtonTap)];
		testTap.numberOfTapsRequired = 10;
		[testButton addGestureRecognizer:testTap];
		
		// TODO: Ton - Intro: prodButton init
		prodButton = [UIButton buttonWithType:UIButtonTypeCustom];
		prodButton.backgroundColor = [UIColor clearColor];
		UITapGestureRecognizer *prodTap =
		[[UITapGestureRecognizer alloc] initWithTarget:self
												action:@selector(prodButtonTap)];
		prodTap.numberOfTapsRequired = 10;
		[prodButton addGestureRecognizer:prodTap];
...
- (void)animateIn {
	...
	// TODO: Ton - Intro: introImageView center
	introImageView.center = CGPointMake(self.view.bounds.size.width / 2.0, self.view.bounds.size.height / 2.0);
...
- (void)loadGL
{
	...
	if (/*[[UIApplication sharedApplication] applicationState] != UIApplicationStateBackground*/true && !_isOpenGLLoaded)
	{
		...
		// TODO: Ton: Background image layout
		...
		// TODO: Ton - Intro: introImageView frame
		int introSize = size * 0.8;
		introImageView.frame = CGRectMake(
										  self.view.bounds.size.width / 2 - introSize / 2,
										  height,
										  introSize,
										  introSize
										  );
...
- (void)viewDidLoad
{
	...
	_pageScrollView.contentSize = CGSizeMake(headlines.count * self.view.bounds.size.width, self.view.bounds.size.height);
	...
	{
		RMIntroPageView *p = [[RMIntroPageView alloc]initWithFrame:CGRectMake(i * self.view.bounds.size.width, 0, self.view.bounds.size.width, 0) headline:[headlines objectAtIndex:i] description:[_descriptions objectAtIndex:i] titleColor:_titleColor descriptionColor:_descriptionColor  accentColor:_accentColor];
		p.opaque = true;
	...
	[_pageControl setNumberOfPages:headlines.count];
	...
	// TODO: Ton - Intro: introImageView add
	[self.view addSubview:introImageView];
	
	
	// TODO: Ton - Intro: testButton add
	[self.view addSubview:testButton];
	// TODO: Ton - Intro: prodButton add
	[self.view addSubview:prodButton];
}

// TODO: Ton - Intro: intro images
- (UIView *)createAnimationSnapshot {
	//UIImage *image = _glkView.snapshot;
	UIImage *image = [UIImage imageNamed:@"Ton/Intro1"];
...
- (void)updateLayout
{
	...
	// TODO: Ton - Intro: move down page indicator
	//_pageControl.frame = CGRectMake(0, pageControlY + 32, self.view.bounds.size.width, 7);
	NSLog(@"pageControlY: %f", pageControlY);
	_pageControl.frame = CGRectMake(
									0,
									self.view.bounds.size.height - startButtonY - 32,
									self.view.bounds.size.width,
									7
									);
	_glkView.frame = CGRectChangedOriginY(_glkView.frame, glViewY - statusBarHeight);
	// TODO: Ton - Intro: introImageView frame
	introImageView.frame = CGRectChangedOriginY(introImageView.frame, (glViewY * 1.35) - statusBarHeight);
	// TODO: Ton - Intro: testButton frame
	testButton.frame = CGRectMake(4, 68, 120, 120);
	// TODO: Ton - Intro: testButton frame
	prodButton.frame = CGRectMake(self.view.bounds.size.width - 120 - 4, 68, 120, 120);
	...
	_pageScrollView.contentSize=CGSizeMake(headlines.count * self.view.bounds.size.width, 150);
...
// TODO: Ton - Intro: scrollViewDidEndDragging
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
	...
	if (justEndDragging)
	{
		...
		if (_currentPage > (headlines.count - 1)){
			return;
		}
		// TODO: Ton - Intro: change intro image when page scroll end
		NSString *imageName ;
		
		if (_currentPage == 0) {
			imageName = @"Ton/AppIcon";
			// fade out background
			// we'll animate alpha instead of toggling hidden directly
		} else {
			imageName = [NSString stringWithFormat:@"Ton/Intro%ld", (long)_currentPage];
		}
		
		UIImage *image = [UIImage imageNamed:imageName];
		
		dispatch_async(dispatch_get_main_queue(), ^{
			// Background image handling: set image and animate alpha
...
//			}
			...
			// TODO: Ton - Intro: introImageView animate
			if (introImageView != nil) {
				[UIView transitionWithView:introImageView
								  duration:0.6
