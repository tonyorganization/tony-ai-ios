        ...
        return { item, params, neighbors in
            ...
            return (ListViewItemNodeLayout(contentSize: contentSize, insets: insets), { [weak self] in
                if let strongSelf = self {
                    ...
                    case .ads:
                        itemTitle = item.presentationData.strings.Stars_Intro_Transaction_AppNameAds_Title
                        itemSubtitle = item.presentationData.strings.Stars_Intro_Transaction_AppNameAds_Subtitle
                    case .apiLimitExtension:
                        itemTitle = item.presentationData.strings.Stars_Intro_Transaction_AppNameBotApi_Title
                        if let floodskipNumber = item.transaction.floodskipNumber {
                            itemSubtitle = item.presentationData.strings.Stars_Intro_Transaction_AppNameBotApi_Messages(floodskipNumber)
                        } else {
                            itemSubtitle = nil
                        }
                    case .unsupported:
